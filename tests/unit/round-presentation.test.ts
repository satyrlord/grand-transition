import type { SkinSpeechProfile } from '../../src/audio/skin-speech-profile.ts';
import { describe, expect, test, vi } from 'vitest';
import {
  RoundPresentation,
  type RoundPresentationFrame,
} from '../../src/app/round-presentation.ts';
import { GameSpeech } from '../../src/audio/game-speech.ts';
import type { SpeechRequest } from '../../src/audio/speech-port.ts';
import type { AudioPort } from '../../src/audio/audio-port.ts';
import { defaultSettings } from '../../src/persistence/codecs/settings-codec.ts';
import { component, publicPlayer, publicOpponent, resolution } from '../fixtures/narration.ts';
...
function harness(enabled = true) {
  ...
  const voice = {
    ...
    pause: vi.fn(),
    resume: vi.fn(),
    prepare: vi.fn(() => ({ accepted: true })),
    speak: (request: SpeechRequest) => {
      requests.push(request);
      return { accepted: true };
    },
  };
  const speech = new GameSpeech(voice);
  speech.userGesture();
  const play = vi.fn();
  const completed = vi.fn();
  let frame: RoundPresentationFrame | null = null;
  const controller = new RoundPresentation(
    speech,
    { play } as unknown as AudioPort,
    {
      now: () => now,
  ...
  const input = {
    ...
    firstSpeakerId: 'two',
    components: { one: [component(10)], two: [component(20)] },
    settings: { ...defaultSettings, speechEnabled: enabled },
    voices: {
      one: {
        provider: 'neural',
...
describe('reference round presentation', () => {
  test('last finisher recites, receives inline scoring, lands damage, then yields to the other speaker', () => {
    const h = harness();
    const original = JSON.stringify(h.input);
    h.controller.start(h.input);
    expect(h.frame()?.phase).toBe('preparing');
    expect(h.requests[0]?.text).toBe(publicOpponent.insultText);
    expect(h.frame()?.impact).toEqual({ playerId: 'one', amount: 20, prideAfter: 80 });
    expect(h.voice.prepare).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ text: publicPlayer.insultText }),
    );
    ...
    h.requests[0]!.onSegment!(0);
    expect(h.frame()?.components).toHaveLength(0);
    h.requests[0]!.onSegment!(1);
    expect(h.frame()?.components).toEqual([component(20)]);
    h.requests[0]!.onEnd!();
    expect(h.frame()?.total).toBe(20);
    h.advance(400);
    expect(h.frame()?.phase).toBe('strike');
    h.advance(200);
    expect(h.frame()?.phase).toBe('points');
    h.advance(199);
    expect(h.frame()?.pride.one).toBe(100);
    h.advance(1);
    expect(h.frame()?.phase).toBe('damage');
    expect(h.frame()?.pride.one).toBe(80);
    expect(h.frame()?.cues.one?.stateId).toBe('heavy-hit');
    expect(h.play).toHaveBeenCalledExactlyOnceWith('hit-heavy');
    h.advance(199);
    expect(h.requests).toHaveLength(1);
    h.advance(1);
    expect(h.requests[1]?.text).toBe(publicPlayer.insultText);
    h.requests[1]!.onStart!();
    h.requests[1]!.onSegment!(1);
    h.requests[1]!.onEnd!();
    h.advance(1000);
  ...
  test('Pause preserves the exact remaining presentation time and blocks stale cancellation callbacks', () => {
    ...
    h.controller.pause();
    h.advance(100000);
    expect(h.frame()?.phase).toBe('total');
    h.controller.resume();
    h.advance(299);
    expect(h.frame()?.phase).toBe('total');
    h.advance(1);
  ...
  test('silent delivery follows the same ordered sequence without a mandatory Continue action', () => {
    ...
    h.controller.start(h.input);
    expect(h.frame()?.phase).toBe('reciting');
  ...
  test('a continuation thinks without speaking a fragment', () => {
    ...
    h.controller.start({
      ...h.input,
      resolution: resolution({
        one: { ...publicPlayer, opponentOutgoingDamage: 0, prideAfter: 100 },
        two: {
          ...publicOpponent,
  ...
  test('an incomplete construction reports zero damage without calling speech', () => {
    ...
    expect(h.frame()?.phase).toBe('hesitating');
    expect(h.frame()?.outcome).toEqual({
  ...
  test.each([0, 1, 15, 16, 100])(
    'lands damage %s at the impact boundary without suppressing the second speaker',
    (damage) => {
      const h = harness();
      h.controller.start({
        ...h.input,
        resolution: resolution({
          one: {
            ...publicPlayer,
            opponentOutgoingDamage: damage,
            prideAfter: Math.max(0, 100 - damage),
          },
      ...
      h.advance(1);
      expect(h.frame()?.pride.one).toBe(Math.max(0, 100 - damage));
      expect(h.frame()?.cues.one?.stateId).toBe(
        damage === 0 ? 'idle' : damage < 16 ? 'light-hit' : 'heavy-hit',
      );
      expect(h.play.mock.calls.flat()).toEqual(
        damage === 0 ? [] : [damage < 16 ? 'hit-light' : 'hit-heavy'],
      );
      h.advance(200);
  ...
  test('bonus cues follow completed phrases once without replaying the accepted comeback cue', () => {
    ...
    h.controller.start({
      ...
      resolution: resolution({
        ...
        two: {
          ...
          comebackClosingLine: 'The record is closed.',
          score: {
            finalDamage: 20,
            unroundedTotal: 20,
            combo: null,
    ...
    expect(h.frame()?.comebackActive).toBe(false);
    voice.onSegment!(1);
    expect(h.play.mock.calls.flat()).toEqual(['combo']);
    expect(h.frame()?.comebackActive).toBe(false);
  ...
  test('a later-clause weakness waits for its component narration anchor', () => {
    ...
    expect(h.play).not.toHaveBeenCalled();
    expect(h.frame()?.emphasis).not.toContainEqual(expect.objectContaining({ kind: 'weakness' }));
    voice.onEnd!();
  ...
  test('direct grammar knockout shows only the damage stance, then completes after 520 ms', () => {
    ...
    expect(h.requests).toHaveLength(0);
    expect(h.frame()?.cues.one).toMatchObject({ stateId: 'grammar-mistake', hold: true });
  ...
  test('cliffhanger keeps score points separate from scaled Pride loss and hit severity', () => {
    ...
    h.controller.start({
      ...
      resolution: {
        ...resolution({
          one: { ...publicPlayer, opponentOutgoingDamage: 100, prideAfter: 0 },
          two: { ...publicOpponent, outgoingDamage: 5 },
        }),
        suddenDeath: true,
      },
    });
    h.requests[0]!.onEnd!();
    expect(h.frame()?.total).toBe(5);
    h.advance(800);
    expect(h.frame()?.damage).toEqual({ playerId: 'one', amount: 100 });
    expect(h.frame()?.pride.one).toBe(0);
    expect(h.frame()?.cues.one?.stateId).toBe('heavy-hit');
    expect(h.play).toHaveBeenCalledExactlyOnceWith('hit-heavy');
  });
});
