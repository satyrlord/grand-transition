...
import { omitFallbackSpeechWasmPlugin } from '../../vite.config.ts';
...
test('leaves only the verified tts WASM in the production bundle', () => {
  ...
  const bundle: Record<string, { type: 'asset' | 'chunk'; fileName: string }> = {
    'assets/ort-wasm-simd-threaded-DDx2apAW.wasm': asset(
      'assets/ort-wasm-simd-threaded-DDx2apAW.wasm',
    ),
    'assets/ort-wasm-simd-threaded.asyncify-CsxMlmQ8.wasm': asset(
      'assets/ort-wasm-simd-threaded.asyncify-CsxMlmQ8.wasm',
    ),
