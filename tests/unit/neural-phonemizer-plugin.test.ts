...
const load = (plugin: Plugin, id: string) =>
  (plugin.load as (id: string) => Promise<string | undefined>)(id);
...
test('concurrent worker instances parse once and each resolves the complete virtual graph', async () => {
  const { neuralPhonemizerPlugin } = await import('../../tools/neural-phonemizer-plugin.ts');
  const first = neuralPhonemizerPlugin();
  ...
  while (pending.length) {
    for (const match of pending
      .pop()!
      .matchAll(/import\("(virtual:grand-transition-phonemizer:[^"]+)"\)/gu)) {
      const id = match[1]!;
      if (seen.has(id)) continue;
      seen.add(id);
      expect(resolve(first, id)).toBe('\0' + id);
      expect(resolve(second, id)).toBe('\0' + id);
      const left = await load(first, '\0' + id);
      const right = await load(second, '\0' + id);
      expect(left).toBeTypeOf('string');
      expect(right).toBe(left);
      pending.push(left!);
    }
  }
  expect(seen.has(prefix + 'engine')).toBe(true);
