...
import { LocalNeuralSpeech } from '../../src/audio/neural-speech.ts';
import type { NeuralSpeechMessage } from '../../src/audio/speech-port.ts';
...
function harness(
  ...
  const speech = new LocalNeuralSpeech(changed, {
    ...options,
    supported,
  ...
  const emit = (message: NeuralSpeechMessage) =>
    workers.at(-1)!.onmessage!({ data: message } as MessageEvent<NeuralSpeechMessage>);
  async function initialize() {
    const ready = speech.initialize();
    emit({ type: 'booted' });
    emit({ type: 'ready', voices });
    await ready;
  }
  const prepared = (id: number): NeuralSpeechMessage => ({
    type: 'speech',
    id,
...
describe('local neural speech', () => {
  test('an exact voice requirement ends silently when that voice is missing', async () => {
    const h = harness({
      strictVoiceUri: true,
      acceptsLanguage: (language) => language === 'ro-RO',
    });
    await h.initialize();
    const onError = vi.fn();
    h.speech.speak({
    ...
    expect(
      h.workers[0]!.postMessage.mock.calls.filter(([message]) => message.type === 'synthesize'),
    ).toEqual([]);
  ...
  test('GPU initialization expires after inactivity but progress keeps loading alive', async () => {
    vi.useFakeTimers();
    const h = harness({ initializationTimeoutMs: 120000 });
    const ready = h.speech.initialize(false);
    h.emit({ type: 'booted' });
    vi.advanceTimersByTime(119000);
    h.emit({ type: 'progress', loaded: 10, total: 100 });
    vi.advanceTimersByTime(119000);
    expect(h.speech.status).toBe('loading');
  ...
  test('Piper reuses its filter across chunks and clears its tail on cancellation', async () => {
    ...
    h.emit(h.prepared(1));
    h.emit(h.prepared(2));
    expect(h.filters).toHaveLength(2);
    h.speech.cancel();
    expect(h.filters.every((filter) => filter.disconnect.mock.calls.length === 1)).toBe(true);
    h.speech.speak({ text: 'Again.', language: 'en-GB' });
    await Promise.resolve();
    h.emit(h.prepared(3));
  ...
  test('GPU playback bypasses Piper filtering', async () => {
    const h = harness({ piperClarity: false });
    await h.initialize();
    h.speech.speak({ text: 'Public.', language: 'en-GB' });
    await Promise.resolve();
    h.emit(h.prepared(1));
  ...
  test('reports GPU playback only when its audio clock starts and suppresses canceled startup', async () => {
    vi.useFakeTimers();
    const h = harness({ sampleRate: 24000, startupBufferSeconds: 0.75 });
    await h.initialize();
    ...
    h.speech.resume();
    await Promise.resolve();
    h.context.currentTime = 10.75;
    vi.advanceTimersByTime(50);
    expect(
      onDiagnostic.mock.calls.filter(([event]) => event.type === 'playback-start'),
    ).toHaveLength(1);
    h.speech.cancel();
    h.emit({ type: 'error', id: 2 });
  ...
  test('GPU multi-chunk playback buffers once, preserves marker offsets, and cancels scheduled audio', async () => {
    ...
    const ready = h.speech.initialize();
    h.emit({ type: 'booted' });
    h.emit({
      type: 'ready',
      voices: [{ voiceURI: 'kokoro:bm_george', name: 'George', lang: 'en-GB', default: true }],
    });
    await ready;
    h.speech.speak({
      text: 'One. Two.',
    ...
    await Promise.resolve();
    expect(h.workers[0]!.postMessage).toHaveBeenLastCalledWith(
    ...
    h.speech.cancel('navigation');
    expect(h.sources.every((source) => source.stop.mock.calls.length === 1)).toBe(true);
    h.speech.dispose();
  });
  ...
  test('a fully prepared GPU delivery starts immediately even when it has multiple chunks', async () => {
    ...
    h.speech.speak(request);
    expect(h.sources[0]!.start).toHaveBeenCalledExactlyOnceWith(10);
    expect(h.sources[1]!.start).toHaveBeenCalledExactlyOnceWith(11);
    h.speech.dispose();
  });
  ...
  test('streams contiguous chunks, prioritizes the current speaker, and completes only after the final audio', async () => {
    ...
    const ended = vi.fn();
    const first = {
      text: 'Your office failed, and your audit failed.',
      language: 'en-GB',
      segments: ['Your office ', 'failed,', ' and ', 'your audit failed.'],
      chunkStarts: [0, 2],
      onEnd: ended,
    };
    const future = { text: 'The public record disagrees.', language: 'en-GB' };
    h.speech.speak(first);
    h.speech.prepare(future);
    await Promise.resolve();
    h.emit(h.prepared(1));
    expect(h.sources).toHaveLength(1);
    expect(h.workers[0]!.postMessage).toHaveBeenLastCalledWith(
      expect.objectContaining({ id: 2, segments: first.segments.slice(2) }),
    );
    h.context.currentTime = 10.5;
    h.emit(h.prepared(2));
    expect(h.sources[1]!.start).toHaveBeenCalledExactlyOnceWith(12);
    expect(h.workers[0]!.postMessage).toHaveBeenLastCalledWith(
      expect.objectContaining({ id: 3, segments: [future.text] }),
    );
    h.emit(h.prepared(3));
  ...
  test('prepares the next delivery during playback, holds it through the reaction, and reuses its audio', async () => {
    ...
    await h.initialize();
    const second = {
      text: 'The public record disagrees.',
      language: 'en-GB',
      voiceUri: 'piper:vctk-p225',
      rate: 1.2,
      pitch: 0.9,
    };
    h.speech.speak({ text: 'Your office failed.', language: 'en-US' });
    h.speech.prepare(second);
    await Promise.resolve();
    expect(
      h.workers[0]!.postMessage.mock.calls.filter(([message]) => message.type === 'synthesize'),
    ).toHaveLength(1);
    ...
    expect(h.speech.status).toBe('ready');
    const ended = vi.fn();
    h.speech.speak({ ...second, volume: 0.4, onEnd: ended });
    expect(h.sources).toHaveLength(2);
    expect(
      h.workers[0]!.postMessage.mock.calls.filter(([message]) => message.type === 'synthesize'),
    ).toHaveLength(2);
  ...
  test('replacement waits for canceled computation without replaying its audio or leaving the queue unbounded', async () => {
    const h = harness();
    await h.initialize();
    h.speech.prepare({ text: 'Old public insult.', language: 'en-US' });
    await Promise.resolve();
    h.speech.cancel();
    h.speech.speak({ text: 'New public insult.', language: 'en-US' });
    await Promise.resolve();
    expect(
      h.workers[0]!.postMessage.mock.calls.filter(([message]) => message.type === 'synthesize'),
    ).toHaveLength(1);
    ...
    expect(h.sources).toHaveLength(0);
    expect(h.workers[0]!.postMessage).toHaveBeenLastCalledWith(
      expect.objectContaining({ segments: ['New public insult.'] }),
    );
    h.emit(h.prepared(2));
    expect(h.sources).toHaveLength(1);
    h.speech.dispose();
  ...
  test.each(['rate', 'pitch', 'voiceUri', 'language', 'segments'] as const)(
    'does not reuse preparation with a different %s',
    async (field) => {
      const h = harness();
      await h.initialize();
      const request = {
      ...
      h.speech.speak({ ...request, [field]: values[field] });
      await Promise.resolve();
      expect(h.sources).toHaveLength(0);
      h.emit(h.prepared(2));
      expect(h.sources).toHaveLength(1);
  ...
  test('cancellation discards prepared audio and preparation alone never emits delivery callbacks', async () => {
    ...
    await h.initialize();
    const onError = vi.fn();
    const request = { text: 'Public.', language: 'en-US', onError };
    h.speech.prepare(request);
    await Promise.resolve();
    h.speech.cancel();
    h.emit(h.prepared(1));
  ...
  test('waits for worker readiness before loading and publishes model progress and local voices', async () => {
    ...
    const ready = h.speech.initialize();
    expect(h.context.resume).toHaveBeenCalledOnce();
    expect(h.workers[0]!.postMessage).not.toHaveBeenCalled();
    h.emit({ type: 'booted' });
    expect(h.workers[0]!.postMessage).toHaveBeenCalledExactlyOnceWith({
      type: 'load',
      baseUrl: '/grand-transition/tts/piper/',
    });
    h.emit({ type: 'progress', loaded: 4, total: 10 });
    expect(h.speech.progress).toBe(0.4);
    h.emit({ type: 'ready', voices });
    expect(await ready).toBe(true);
    expect(h.speech.status).toBe('ready');
    expect(h.speech.voices).toEqual(voices);
    h.speech.dispose();
    expect(h.workers[0]!.terminate).toHaveBeenCalledOnce();
  });
  ...
  test('uses generated sample timing for start, phrase markers, pause, and completion', async () => {
    ...
    h.speech.speak({
      text: 'Your office failed.',
      segments: ['Your office ', 'failed.'],
      language: 'en-US',
      voiceUri: 'piper:vctk-p225',
      rate: 1.4,
      pitch: 0.9,
    ...
    h.emit(h.prepared(1));
    expect(h.sources[0]!.start).toHaveBeenCalledExactlyOnceWith(10);
    h.context.currentTime = 10.4;
    vi.advanceTimersByTime(400);
    expect(events.onStart).toHaveBeenCalledOnce();
    expect(events.onSegment).toHaveBeenCalledExactlyOnceWith(0);
    h.speech.pause();
    vi.advanceTimersByTime(1000);
    expect(events.onSegment).toHaveBeenCalledTimes(1);
    h.speech.resume();
    await Promise.resolve();
    h.context.currentTime = 10.95;
  ...
  test('cancellation discards generated responses and stale completion events', async () => {
    ...
    await h.initialize();
    const ended = vi.fn();
    h.speech.speak({ text: 'Public.', language: 'en-US', onEnd: ended });
    await Promise.resolve();
    h.speech.cancel();
    h.emit(h.prepared(1));
    expect(h.sources).toHaveLength(0);
    h.speech.speak({ text: 'Public again.', language: 'en-US', onEnd: ended });
    await Promise.resolve();
    h.emit(h.prepared(2));
    const stale = h.sources[0]!.onended!;
    h.speech.cancel();
    stale();
    expect(ended).not.toHaveBeenCalled();
  ...
  test('unavailable, muted, unsupported-language, and failed-model states stay silent', async () => {
    ...
    expect(h.speech.speak({ text: 'Public.', language: 'ro-RO' }).reason).toBe(
    ...
    expect(h.speech.available).toBe(false);
    expect(h.sources).toHaveLength(0);
    h.speech.dispose();
    const missing = harness();
    missing.supported.mockReturnValue(false);
    expect(await missing.speech.initialize()).toBe(false);
    expect(missing.workers).toHaveLength(0);
  });
  ...
  test('a stalled inference releases the worker and signals silent fallback within one minute', async () => {
    ...
    expect(h.speech.status).toBe('unavailable');
    expect(h.workers[0]!.terminate).toHaveBeenCalledOnce();
    expect(onDiagnostic.mock.calls.map(([event]) => event.type)).toEqual([
      'synthesis-start',
      'timeout',
      'error',
      'cancel',
