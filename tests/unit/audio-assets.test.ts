...
const tools = await import(pathToFileURL(path.resolve('tools/audio-assets.ts')).href);
...
describe('audio asset measurements', () => {
  ...
  test.each([-17, -15])('accepts the music loudness boundary %s', (value) => {
    expect(() =>
      tools.validateMeasurement({ ...valid, integratedLufs: value }, 'music', 'ogg'),
    ).not.toThrow();
  });
  test('accepts the effect true-peak boundary', () => {
    expect(() =>
      tools.validateMeasurement({ ...valid, truePeakDbfs: -1 }, 'effect', 'ogg'),
    ).not.toThrow();
  });
  ...
  test('rejects overlapping and out-of-bounds music edits from the same source', () => {
    ...
    expect(() => tools.validateMusicDefinitions(definitions)).toThrow(
      /outside the pinned recording/u,
    );
  });
...
describe('complete music loops', () => {
  test('rejects a truncated phrase and an audible waveform jump', () => {
    expect(() =>
      tools.validateLoopMeasurement(
        { durationSeconds: 60, seamJump: 0 },
        132.41379310344828,
        'ogg',
      ),
    ).toThrow(/duration/u);
    expect(() =>
      tools.validateLoopMeasurement({ durationSeconds: 132, seamJump: 0.2 }, 132, 'ogg'),
    ).toThrow(/discontinuity/u);
  });
  ...
  test('repairs a stereo seam without removing frames or fading the body', () => {
    ...
    for (const channel of [0, 1]) {
      expect(repaired.readFloatLE(channel * 4)).toBe(
        repaired.readFloatLE(repaired.length - 8 + channel * 4),
      );
    }
  ...
  test('extends the source with a distinct breakdown and preserves the original first phrase', () => {
    ...
    for (let frame = 0; frame < 4800; frame += 1) {
      for (const channel of [0, 1])
        pcm.writeFloatLE(Math.sin(frame * 0.9) * 0.3, frame * 8 + channel * 4);
    }
    ...
    expect(arranged.subarray(pcm.length, pcm.length + 2400 * 8)).not.toEqual(
      arranged.subarray(0, 2400 * 8),
    );
    expect(arranged.subarray(pcm.length + 2400 * 8)).toEqual(
      arranged.subarray(2400 * 8, pcm.length),
    );
  });

  test.each(['modern-debate-studio', 'palace-press-hall', 'influencer-campaign-livestream'])(
    'ships a complete continuous phrase in all formats for %s',
    (sceneId) => {
      const definition = tools.sceneMusicDefinitions.find(
        (entry: { sceneId: string }) => entry.sceneId === sceneId,
      );
      for (const format of ['wav', 'ogg', 'mp3']) {
        const measured = tools.loopMeasurements(`src/assets/audio/${sceneId}-theme.${format}`);
        expect(() =>
          tools.validateLoopMeasurement(measured, definition.duration, format),
        ).not.toThrow();
      }
    },
  );
...
describe('audio asset inventory', () => {
  ...
  test.each(['owner', 'license', 'source', 'sceneId', 'id', 'kind'])(
  ...
  test('ships one distinct manifest-backed treatment per scene and ten effects, with no room tone', () => {
    ...
    expect(
      manifest.assets.filter((asset: { kind: string }) => asset.kind === 'effect'),
    ).toHaveLength(10);
    expect(manifest.assets).toHaveLength(18);
    expect(manifest.assets.some((asset: { id: string }) => asset.id.includes('room-tone'))).toBe(
      false,
    );
    expect(new Set(music.map((asset: { id: string }) => asset.id)).size).toBe(8);
    const sceneMusic = new Map(
      music.map((asset: { id: string; sceneId: string }) => [asset.sceneId, asset.id]),
    );
    ...
    expect(
      new Set(playableMusic.map((asset: { sourceSha256: string }) => asset.sourceSha256)).size,
    ).toBe(7);
    expect(
      playableMusic.every((asset: { license: string }) =>
        ['CC0-1.0', 'CC-BY-3.0', 'CC-BY-4.0', 'Public domain'].includes(asset.license),
      ),
    ).toBe(true);
    expect(
      playableMusic.find(
        (asset: { sceneId: string }) => asset.sceneId === 'influencer-campaign-livestream',
      )?.treatment,
    ).toMatch(/trap/iu);
    expect(
      playableMusic.find(
        (asset: { sceneId: string }) => asset.sceneId === 'civic-cypher-boxing-ring',
      )?.treatment,
    ).toMatch(/boom-bap/iu);
  });
  ...
  test('keeps the established Scene 1 recording bytes unchanged', () => {
    const manifest = JSON.parse(original);
    const sceneOne = manifest.assets.find(
      (asset: { id: string }) => asset.id === 'transition-era-television-studio-theme',
    );
  ...
  test('credits every music source and creator in README and CREDITS', async () => {
    ...
    for (const asset of manifest.assets.filter(
      (entry: { kind: string }) => entry.kind === 'music',
    )) {
