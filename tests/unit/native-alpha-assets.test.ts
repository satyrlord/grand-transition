...
import { inspectAlpha as inspectCharacter } from '../../tools/validate-character-assets.ts';
import { inspectAlpha as inspectScene } from '../../tools/validate-scene-assets.ts';
import { hasNativeAlphaProvenance } from '../../tools/asset-pixels.ts';
import { validateStateAsset } from '../../tools/validate-character-states.ts';
...
test('shipped native Local Baron portrait satisfies production border and contour checks', async () => {
  ...
  await expect(
    inspectCharacter(input, 'shipped Local Baron', { nativeAlpha: true }),
  ).resolves.toBeUndefined();
});

test.each(['avif', 'webp'] as const)(
  'small native %s encoding keeps the outer border transparent',
  async (format) => {
    const input = await readFile(
      path.resolve('src/assets/characters/county-baron--municipal-patron.png'),
    );
    ...
    await expect(
      inspectCharacter(output, `128px ${format}`, { nativeAlpha: true }),
    ).resolves.toBeUndefined();
  },
);
...
async function fixture(interior = 250, edge = 128, green = false): Promise<Buffer> {
  ...
  return sharp(pixels, { raw: { width: 100, height: 100, channels: 4 } })
    .png()
    .toBuffer();
}

async function invalidTopologyFixture(kind: 'border' | 'detached' | 'haze'): Promise<Buffer> {
  const decoded = await sharp(await fixture())
    .raw()
...
test.each([250, 253, 254, 255])(
  'native character and foreground alpha accepts %s interiors and green materials',
  async (interior) => {
    ...
    await expect(
      inspectScene(input, true, 'native', { nativeAlpha: true }),
    ).resolves.toBeUndefined();
    ...
    await expect(inspectScene(input, false, 'back', { nativeAlpha: true })).rejects.toThrow(
      /fully opaque/u,
    );
  },
);
...
test.each([
  ...
  [0, 0],
  [128, 250],
])('rejects native empty, translucent, or edgeless art (%s, %s)', async (interior, edge) => {
...
test('native adoption rejects detached alpha before stamping workflow metadata', async () => {
  ...
  try {
    ...
    await execFileAsync(process.execPath, [
      ...
      file,
      '--source',
      'Synthetic native-alpha test fixture.',
    ]);
...
test('native adoption preserves decoded pixels and records provenance without green-key claims', async () => {
  ...
  try {
    ...
    await expect(
      execFileAsync(process.execPath, [script, 'validate', root]),
    ...
    const asset = {
      id: 'native-state',
      source: record(stamped, 'native.png', 'png'),
