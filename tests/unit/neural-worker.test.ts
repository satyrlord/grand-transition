...
vi.mock('onnxruntime-web/wasm', () => ({
  ...
  InferenceSession: {
    create: async () => ({
      run: async (input: (typeof inference.inputs)[number]) => {
        inference.inputs.push(input);
        return {
          output: { data: new Float32Array(input.input.data.length * 256).fill(0.1), dispose() {} },
          phoneme_durations: {
            data: new Float32Array(input.input.data.length).fill(1),
            dispose() {},
          },
        };
      },
...
async function workerHarness() {
  ...
  const vocabulary = Object.fromEntries(
    Array.from('_^$ abcdefghijklmnopqrstuvwxyz.,!?').map((letter, index) => [letter, [index]]),
  );
    ...
    [
      ...
      new TextEncoder().encode(
        JSON.stringify({
          ...
          espeak: { voice: 'en' },
          phoneme_id_map: vocabulary,
          inference: { noise_scale: 0.333, length_scale: 1, noise_w: 0.333 },
        }),
      ),
    ],
  ...
  await import('../../src/audio/neural-speech-worker.ts');
...
test('long public speech crosses model chunks without losing tokens or segment markers', async () => {
  ...
  const actualTokens = inference.inputs.reduce(
    (sum, input) => sum + (input.input.data.length - 3) / 2,
    0,
  );
...
test.each(['model.onnx', 'ort-wasm-simd-threaded.wasm'])(
  'corrupt local %s fails initialization before inference',
  async (file) => {
    const h = await workerHarness();
    h.files.set(file, new Uint8Array([9, 9, 9]));
    h.send({ type: 'load', baseUrl: h.base });
...
test('a remote model base is rejected without any request', async () => {
  const h = await workerHarness();
  h.send({ type: 'load', baseUrl: 'https://network.invalid/tts/piper/' });
  await vi.waitFor(() => expect(h.messages).toContainEqual({ type: 'error', id: null }));
