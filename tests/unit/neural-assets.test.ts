...
const { validateNeuralAssets } = await import(
  pathToFileURL(path.resolve('tools/neural-speech-assets.ts')).href
);
...
async function fixture() {
  ...
  const manifest = JSON.parse(
    await readFile(path.resolve('public/tts/piper/manifest.json'), 'utf8'),
  );
...
test.each([
  ...
  await writeFile(path.join(root, 'manifest.json'), JSON.stringify(manifest));
  await expect(validateNeuralAssets(root)).rejects.toThrow('manifest is invalid');
});
...
test('rejects duplicate, missing, and unmanifested neural asset paths', async () => {
  ...
  await writeFile(path.join(duplicate.root, 'manifest.json'), JSON.stringify(duplicate.manifest));
  await expect(validateNeuralAssets(duplicate.root)).rejects.toThrow('manifest is invalid');

  const missing = await fixture();
  missing.manifest.files.pop();
  await writeFile(path.join(missing.root, 'manifest.json'), JSON.stringify(missing.manifest));
  await expect(validateNeuralAssets(missing.root)).rejects.toThrow('manifest is invalid');

  const extra = await fixture();
  await writeFile(path.join(extra.root, 'unexpected.bin'), 'unexpected');
  await expect(validateNeuralAssets(extra.root)).rejects.toThrow('invalid inventory');
});
