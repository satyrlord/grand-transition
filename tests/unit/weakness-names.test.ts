...
describe('weakness labels', () => {
  ...
  test('translates a tag whose two labels differ', () => {
    expect(displayWeaknessName('legacy', 'ro-RO')).not.toBe(
      displayWeaknessName('legacy', 'en'),
    );
    expect(displayWeaknessName('accountability', 'ro-RO')).toBe(
      'Responsabilitate',
    );
  });
  ...
  test('shows the English label the setup and score views shipped', () => {
    ...
    expect(displayWeaknessName('firsthand-knowledge', 'en')).toBe(
      'Firsthand-Knowledge',
    );
    // Milestone 005 pins this one label.
    expect(displayWeaknessName('securitate', 'en')).toBe(
  ...
  test('follows the interface locale rather than the game locale', () => {
    ...
    expect(displayWeaknessName('procurement', 'ro-RO')).toBe(
      'Achiziții publice',
    );
  ...
  test('falls back to the tag in title case so an unknown tag stays readable', () => {
    expect(displayWeaknessName('not-a-shipped-weakness', 'ro-RO')).toBe(
      'Not-A-Shipped-Weakness',
    );
  });
});
