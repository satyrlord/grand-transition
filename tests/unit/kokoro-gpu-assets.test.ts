...
const { validateKokoroGpuAssets } = await import(
  pathToFileURL(path.resolve('tools/kokoro-gpu-assets.ts')).href
);
const roots: string[] = [];
...
test('rejects a stale compiled manifest before loading model bytes', async () => {
  ...
  await expect(validateKokoroGpuAssets(root, compiled)).rejects.toThrow(
    'compiled GPU speech manifest is stale',
  );
});

test.each(['sourceSha256', 'modelSha256', 'revision', 'runtime'])(
  'rejects changed %s provenance',
  async (field) => {
    const { root, manifest } = await fixture();
    manifest[field] = 'changed';
    await writeFile(path.join(root, 'manifest.json'), JSON.stringify(manifest));
    await expect(validateKokoroGpuAssets(root)).rejects.toThrow(
      'manifest or file inventory is invalid',
    );
  },
);

test.each(['duplicate', 'missing', 'changed-hash', 'reordered-shards'])(
  'rejects %s inventory',
  async (mode) => {
    const { root, manifest } = await fixture();
