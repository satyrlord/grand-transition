...
test('the shipped manifest declares both Romanian voices, the shared runtime, and its licences', async () => {
  const manifest = JSON.parse(await readFile('public/tts/ro/manifest.json', 'utf8')) as {
    voices: { id: string; default: boolean; model: string; lang: string; license: string }[];
    runtime: { path: string; bytes: number; sha256: string };
    files: ManifestFile[];
  };
  expect(manifest.voices.map((voice) => voice.id)).toEqual(['ro_RO-mihai-medium', 'ro_RO-liana-medium']);
  expect(manifest.voices[0]).toMatchObject({ default: true, model: 'mihai/model.onnx', lang: 'ro-RO' });
  expect(manifest.voices[1]).toMatchObject({ default: false, model: 'liana/model.onnx', license: 'CC BY-NC 4.0' });
