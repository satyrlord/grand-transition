...
const { validateRomanianSpeechAssets } = await import(
  pathToFileURL(path.resolve('tools/romanian-speech-assets.mjs')).href
);
const roots: string[] = [];
...
async function fixture(mutate: (manifest: FixtureManifest) => void = () => {}): Promise<string> {
  ...
  roots.push(root);
  const manifest = JSON.parse(await readFile('public/tts/ro/manifest.json', 'utf8')) as FixtureManifest;
  mutate(manifest);
...
test('rejects a manifest that no longer matches the pinned inventory', async () => {
  const root = await fixture((manifest) => { manifest.files[0]!.sha256 = 'f'.repeat(64); });
  await expect(validateRomanianSpeechAssets(root)).rejects.toThrow('The Romanian speech asset manifest is invalid.');
});

test.each([
  ['voice identity', (manifest: FixtureManifest) => { manifest.voices[1]!.id = 'ro_RO-other-medium'; }],
  ['voice license', (manifest: FixtureManifest) => { manifest.voices[1]!.license = 'MIT'; }],
  ['runtime path', (manifest: FixtureManifest) => { manifest.runtime.path = '../other/runtime.wasm'; }],
...
test('the shipped manifest declares both Romanian voices, the shared runtime, and its licences', async () => {
  const manifest = JSON.parse(await readFile('public/tts/ro/manifest.json', 'utf8')) as {
    voices: { id: string; default: boolean; model: { files: string[]; bytes: number; sha256: string }; lang: string; license: string }[];
    runtime: { path: string; bytes: number; sha256: string };
    files: ManifestFile[];
  };
  expect(manifest.voices.map((voice) => voice.id)).toEqual(['ro_RO-mihai-medium', 'ro_RO-liana-high']);
  expect(manifest.voices[0]).toMatchObject({ default: true, model: { files: ['mihai/model.onnx'] }, lang: 'ro-RO' });
  expect(manifest.voices[1]).toMatchObject({ default: false,
    model: { files: ['liana/model-01.bin', 'liana/model-02.bin'] }, license: 'CC BY-NC 4.0' });
  ...
  expect(manifest.files.map(({ path: name }) => name)).not.toContain('liana/model.onnx');
