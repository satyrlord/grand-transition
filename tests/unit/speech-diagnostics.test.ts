...
} from '../../src/audio/speech-diagnostics.ts';
...
test('speech diagnostics retain a bounded timed tail and explicitly count dropped events', () => {
  let now = 100;
  const recorder = new SpeechDiagnostics(() => now);
  recorder.reset();
  ...
  expect(document.droppedEvents).toBe(2);
  expect(document.events).toHaveLength(1000);
  expect(document.events[0]!.elapsedMs).toBe(3);
  document.events.length = 0;
  expect(recorder.snapshot().events).toHaveLength(1000);
  recorder.finish(true);
  expect(recorder.snapshot().status).toBe('finished');
  ...
  recorder.finish(true);
  expect(recorder.snapshot().status).toBe('interrupted');
});

test('diagnostics reject arbitrary text, error details, and invalid timing metadata', () => {
  const recorder = new SpeechDiagnostics(() => 0);
  recorder.reset();
  ...
  expect(recorder.snapshot().events).toEqual([]);
  expect(
    speechDiagnosticsSchema.safeParse({
      ...recorder.snapshot(),
      events: [{ ...event, elapsedMs: -1 }],
    }).success,
  ).toBe(false);
});
