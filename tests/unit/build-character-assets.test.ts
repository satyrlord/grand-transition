...
import * as characterBuilder from '../../tools/build-character-assets.ts';
...
beforeAll(async () => {
      ...
      ['alpha', 'beta--alternate'].map(async (id) => [
        id,
        {
          ...
          sourceSha256: createHash('sha256')
            .update(await readFile(path.join(fixture, id + '.png')))
            .digest('hex'),
        },
      ]),
    ),
  ...
  await writeFile(
    path.join(fixture, 'portrait-layout.json'),
    JSON.stringify({ schemaVersion: 1, portraits }),
  );
});
...
describe('character asset builder', () => {
  test('rejects missing, invalid, or stale orientation review before changing outputs', async () => {
    ...
    for (const mutation of ['missing', 'invalid', 'stale']) {
      const layout = JSON.parse(original) as {
      ...
      try {
        ...
        ).rejects.toThrow(/inventory|facing/u);
  ...
  test('waits for active encoders before reporting a failure and stops the remaining queue', async () => {
    ...
    const result = mapWithConcurrency([0, 1, 2], 2, async (value) => {
      ...
      if (value === 0) {
        ...
        throw new Error('failed encoding');
      }
      await gate;
      activeFinished = true;
      return value;
  ...
  test('reproduces the manifest and every runtime variant', async () => {
    const masterNames = ['alpha.png', 'beta--alternate.png'];
    const first = await buildCharacterAssets({
      characterRoot: fixture,
      masterNames,
    });
    const firstManifest = await readFile(path.join(fixture, 'character-manifest.json'), 'utf8');
    ...
    const secondManifest = await readFile(path.join(fixture, 'character-manifest.json'), 'utf8');
    ...
    expect(
      second.assets.flatMap((asset) => asset.variants.map((variant) => variant.sha256)),
    ).toEqual(firstHashes);
  ...
  test('rebuilds only selected character variants and rejects a stale reused cache', async () => {
    const masterNames = ['alpha.png', 'beta--alternate.png'];
    ...
    expect(await readFile(path.join(fixture, 'variants/alpha-128x128.avif'), 'utf8')).not.toBe(
      'replace selected cache',
    );
    expect(selected.assets.find(({ id }) => id === 'alpha')!.source.sha256).toBe(
      layout.portraits.alpha!.sourceSha256,
    );
