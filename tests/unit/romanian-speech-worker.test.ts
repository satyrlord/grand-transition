...
vi.mock('onnxruntime-web/ort-wasm-simd-threaded.mjs?url&no-inline', () => ({
  default: '/grand-transition/assets/runtime.mjs',
}));
...
async function harness(cachedManifest?: unknown) {
  ...
  const files = new Map([
    ...
    ['mihai/config.json', config],
    ['liana/config.json', config],
    ...
    voices: [
      {
        ...
        speakerId: 0,
        default: true,
        model: { files: ['mihai/model.onnx'], bytes: 3, sha256: hash(new Uint8Array([1, 2, 3])) },
      ...
      },
      {
        ...
        model: {
          ...
          bytes: 4,
          sha256: hash(new Uint8Array([4, 5, 6, 7])),
        },
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: URL, options: RequestInit) => {
      ...
      const file = [...files.keys(), 'manifest.json'].find(
  ...
  await import('../../src/audio/romanian-speech-worker.ts');
...
test('revalidates a cached package manifest so a deployed voice change loads the current model', async () => {
  ...
  expect(fetch).toHaveBeenCalledWith(
    expect.objectContaining({ pathname: '/grand-transition/tts/ro/manifest.json' }),
...
test.each([
  ...
  'loads only %s on demand and supplies its exact assembled model to inference',
  async (voice, bytes, paths) => {
    const h = await harness();
    await h.ready();
    ...
    expect(
      progress.every(
        (message, index) =>
          message.total === bytes.length &&
          (index === 0 || message.loaded >= progress[index - 1]!.loaded),
      ),
    ).toBe(true);
    h.speak(voice, 2);
    await vi.waitFor(() =>
      expect(h.messages.filter(({ type }) => type === 'speech')).toHaveLength(2),
    );
...
test.each(['missing', 'corrupt', 'unmanifested', 'wrong-size', 'wrong-digest'] as const)(
  'rejects %s model parts before creating an inference session',
  async (failure) => {
    const h = await harness();
    if (failure === 'missing') h.files.delete('liana/model.onnx');
    if (failure === 'corrupt') h.files.set('liana/model.onnx', new Uint8Array([9, 9, 9, 9]));
    if (failure === 'unmanifested')
      h.manifest.files = h.manifest.files.filter(({ path }) => path !== 'liana/model.onnx');
