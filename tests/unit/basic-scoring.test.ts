...
} from '../../src/engine/basic-scoring.ts';
import {
...
describe('Hollywood Roast clause scoring', () => {
  ...
  test.each([
    'common-predicate-013-past',
    'common-predicate-013-present',
    'common-predicate-013-future',
  ])('the concise %s predicate does not imply a credibility weakness', (predicate) => {
  ...
  test.each([
  ...
  ])('neutral $label clauses do not activate any defender weakness', ({ connector }) => {
    const ids = [
      'common-noun-028',
    ...
    expect(
      result.breakdown.some(
        (item) => item.kind === 'weakness-match' || item.kind === 'weakness-multiplier',
      ),
    ).toBe(false);
  });
  ...
  test('neutral with and its neutral complement add no weakness while a tagged complement still matches', () => {
    ...
    expect(tagged.breakdown).toContainEqual(
      expect.objectContaining({
        kind: 'weakness-match',
        phraseId: 'common-noun-033',
        defenderTag: 'corruption',
      }),
    );
  });
  ...
  test('narration anchors distinguish repeated relation occurrences and shared compound completion', () => {
    ...
    expect(
      extractScoreClauseAnchors(
        analysis(['common-noun-001', 'common-predicate-010-present']),
        phrases,
      ),
    ).toEqual([1]);
    expect(
  ...
  test('supports flavour matches and exact noun-specific score overrides', () => {
    const baseAnalysis = analysis(['common-noun-001', 'common-predicate-010-present']);
  ...
  test('applies a weakness multiplier to the matching clause only', () => {
    expect(
      score(['common-noun-001', 'common-predicate-010-present'], ['restraint']).finalDamage,
    ).toBe(10);
  });
  ...
  test('keeps scene and character restrictions out of damage', () => {
    ...
    expect(result.breakdown.some((item) => item.kind === 'restriction-multiplier')).toBe(false);
  });

  test.each([1, 2, 3, 4, 5] as const)(
    'uses compatibility multiplier %s with the fixed five-point base',
    (multiplier) => {
      ...
      expect([tier(false, false), tier(false, true), tier(true, false), tier(true, true)]).toEqual([
        5,
        5 + multiplier,
        5 + 2 * multiplier,
        5 + 3 * multiplier,
  ...
  test.each([1, 2, 3, 4, 5] as const)(
    'keeps custom bases, modifier points, and weakness separate at multiplier %s',
    (multiplier) => {
      ...
      expect(result.breakdown.filter((item) => item.kind === 'weakness-multiplier')).toHaveLength(
        1,
      );
      expect(result.breakdown).toContainEqual(
        expect.objectContaining({ kind: 'clause-base', amount: 11 }),
      );
    },
  );

  test('keeps a modifier in the preceding clause for weakness scoring', () => {
    const ids = ['common-noun-001', 'common-predicate-010-present', 'common-modifier-001'] as const;
    ...
    expect(result.breakdown.filter((item) => item.kind === 'clause-base')).toHaveLength(1);
    expect(result.breakdown).toContainEqual({
      kind: 'clause-base',
      operation: 'note',
      phraseIds: ids,
  ...
  test('each stacked modifier adds points to the screenshot clause', () => {
    ...
    expect(
      [0, 1, 2, 3].map((count) => score([...core, ...modifiers.slice(0, count)]).finalDamage),
    ).toEqual([5, 7, 9, 11]);
    ...
    expect(
      scoreBasicConstruction({
        ...
        phrases: gameCatalog.phrases,
        defenderWeaknessTags: [],
        balance: { ...basicScoringBalance, modifierPoints: 0 },
      }).finalDamage,
    ).toBe(5);
  });
  ...
  test('stacked modifier tags trigger one weakness multiplier on their clause only', () => {
    const ids = [
      ...
      'common-modifier-009',
      'common-modifier-013',
      'algorithmic-prophet-conjunction-001',
      'common-noun-001',
      'common-predicate-010-present',
    ];
    ...
    const phrases = gameCatalog.phrases.map((phrase) => ({
      ...phrase,
      tags: modifiers.has(phrase.id) ? ['modifier-only'] : [],
    }));
    const result = scoreBasicConstruction({
      analysis: analysis(ids),
      phrases,
      defenderWeaknessTags: ['modifier-only'],
      balance: basicScoringBalance,
    });
  ...
  test('an incomplete sentence deals zero damage and has no clause score', () => {
    ...
    expect(result.breakdown.some((item) => item.kind === 'clause-score')).toBe(false);
  });
