...
import { parseSimulationArguments, runSimulationCommand } from '../../tools/simulate.ts';

describe('simulation command', () => {
  test.each([0, 0xffff_ffff])('accepts boundary seed %s', (seed) => {
    expect(parseSimulationArguments(['--seed', String(seed), '--matches', '1'])).toEqual({
      ok: true,
      value: { seed, matches: 1 },
    });
  });
  ...
  ] as const)('rejects invalid arguments and names %s', (arguments_, option) => {
  ...
  test('repeats concise output and normalized output-file bytes', async () => {
    ...
    const directory = await mkdtemp(path.join(os.tmpdir(), 'grand-transition-simulation-'));
    try {
      const firstPath = path.join(directory, 'first.json');
      const secondPath = path.join(directory, 'second.json');
      await runSimulationCommand(['--seed', '4294967295', '--matches', '2', '--output', firstPath]);
  ...
  test('runs each advanced difficulty through the configured policy', async () => {
    for (const difficulty of ['party-strategist', 'palace-operator']) {
      ...
      expect(
        await runSimulationCommand(
          ['--seed', '22', '--matches', '1', '--difficulty', difficulty],
  ...
  test('returns nonzero output that names the invalid option', async () => {
    const messages: string[] = [];
    expect(
      await runSimulationCommand(['--seed', 'bad', '--matches', '1'], (message) =>
        messages.push(message),
      ),
    ).toBe(1);
    expect(messages).toEqual(['Invalid --seed: Use an unsigned 32-bit integer.']);
  });
});
