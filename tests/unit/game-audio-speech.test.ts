...
import {
  GameSpeech,
  publicNarrationSegments,
  publicNarrationChunkStarts,
} from '../../src/audio/game-speech.ts';
import type { AudioPort } from '../../src/audio/audio-port.ts';
import type { SpeechRequest } from '../../src/audio/speech-port.ts';
import type { MatchTransition } from '../../src/app/match-coordinator.ts';
import { defaultSettings } from '../../src/persistence/codecs/settings-codec.ts';
import { publicPlayer, publicOpponent, resolution } from '../fixtures/narration.ts';
...
describe('public audio event projection', () => {
  test('draft cues fire once while scored outcomes wait for narration', () => {
    ...
    const transition: MatchTransition = {
      state: {} as MatchTransition['state'],
      reaction: null,
  ...
  test.each([false, true])(
    'selecting a phrase projects a public cue with mistake=%s',
    (mistake) => {
      const play = vi.fn();
      new GameAudio({ play } as unknown as AudioPort).accepted(
        {
          ...
          source: 'user',
          actorId: 'one',
          payload: { card: { source: 'shared', cardId: 'card' } },
        },
        {
          state: {} as MatchTransition['state'],
          review: null,
          reaction: mistake
            ? { kind: 'grammar-mistake', playerId: 'one', damage: 3, sequence: 1 }
            : null,
        },
      );
...
describe('finalized public speech', () => {
  test('streams separate scored clauses and Comeback without splitting coordinated nouns', () => {
    const player = {
      ...
      constructionPhrases: [
        ...
        'subject-two',
        'relation-two',
      ].map((phraseId) => ({ phraseId, text: phraseId, source: 'active' as const })),
      ...
      score: {
        ...
        breakdown: [
          {
            kind: 'clause-score' as const,
            operation: 'add' as const,
            phraseIds: ['subject', 'relation'],
            amount: 5,
          },
          {
            kind: 'clause-score' as const,
            operation: 'add' as const,
            phraseIds: ['subject-two', 'relation-two'],
            amount: 5,
          },
        ],
      },
          ...
          breakdown: [
            {
              kind: 'clause-score',
              operation: 'add',
              phraseIds: ['subject', 'relation', 'common-conjunction-001', 'subject-two'],
              amount: 10,
            },
          ],
        },
    ...
    expect(
      publicNarrationChunkStarts({
        ...
        score: {
          ...player.score,
          breakdown: [
            player.score.breakdown[0]!,
            {
              kind: 'clause-score',
              operation: 'add',
              phraseIds: ['subject', 'relation-two'],
              amount: 5,
            },
          ],
        },
  ...
  test('late diagnostics from canceled speech cannot enter a later match or delivery scope', () => {
    ...
    const game = new GameSpeech(port, diagnostic);
    game.userGesture();
    game.deliver(publicPlayer, settings, profile, {}, 3);
    game.cancel('navigation');
    ...
    expect(diagnostic).toHaveBeenLastCalledWith(
      expect.objectContaining({ type: 'playback-start', round: 1, speakerId: 'two' }),
    );
  });
  function harness() {
    const requests: SpeechRequest[] = [];
  ...
  test.each([
    ...
    const accepted = game.deliver(
      {
        ...publicPlayer,
        completeValidInsult: state !== 'incomplete',
        constructionStatus: state === 'carried' ? 'carried' : 'valid',
      },
      {
        ...settings,
        speechEnabled: state !== 'disabled',
        masterVolume: state === 'master-zero' ? 0 : 1,
  ...
  test('preserves public wording and uses the skin voice instead of a retired global preference', () => {
    const { game, requests } = harness();
    game.userGesture();
    game.deliver(
      { ...publicPlayer, comebackClosingLine: 'The microphone disagrees.' },
      { ...settings, masterVolume: 0.5, speechRate: 1.4, speechVoiceUri: 'kokoro:af_heart' },
      { ...profile, pitch: 0.8 },
      {},
    );
    expect(requests[0]).toMatchObject({
      text: 'Your office failed. The microphone disagrees.',
  ...
  test('passes start and monotonic segment events once, then rejects finished or cancelled callbacks', () => {
    const { game, requests, port } = harness();
    game.userGesture();
  ...
  test('rejects invalid public phrase alignment instead of inventing or dropping words', () => {
    expect(() =>
      publicNarrationSegments({
        ...publicPlayer,
        constructionPhrases: [{ phraseId: 'bad', text: 'Unrelated words', source: 'active' }],
      }),
