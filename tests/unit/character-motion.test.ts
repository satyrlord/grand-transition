...
import { characterMotion, projectCharacterCue } from '../../src/app/character-motion.ts';
import type { GameCommand } from '../../src/engine/game-contracts.ts';
import type { MatchResolution, MatchResolutionPlayer } from '../../src/engine/match-lifecycle.ts';

const command = (type: string, actorId = 'one'): GameCommand => ({
  type,
  actorId,
  source: 'user',
  payload: {},
});
const state = (...commands: GameCommand[]) => ({
  commandHistory: commands,
  playerOrder: ['one', 'two'] as const,
});
function player(
  playerId: string,
  overrides: Partial<MatchResolutionPlayer> = {},
...
function review(
  one: Partial<MatchResolutionPlayer>,
  two: Partial<MatchResolutionPlayer> = {},
): MatchResolution {
  return {
    round: 1,
    openingPlayerId: 'one',
...
describe('public character motion projection', () => {
  test('selects only on entry and does not replay entry on later rounds', () => {
    expect(projectCharacterCue(state(command('prepare-round')), 'one', null, null).stateId).toBe(
      'selection',
    );
    expect(
      projectCharacterCue(
        state(command('prepare-round'), command('prepare-round')),
        'one',
        null,
        null,
      ).stateId,
  ...
  test('projects only the accepted actor and does not use private command payloads', () => {
    const input = state({
      ...command('select-phrase'),
      payload: { privateCard: 'secret-unplayed-fixture' },
    });
    ...
    expect(projectCharacterCue(state(command('select-comeback')), 'one', null, null).stateId).toBe(
      'idle',
    );
    expect(projectCharacterCue(state(command('redraw-hand')), 'one', null, null).stateId).toBe(
      'idle',
    );
  });
