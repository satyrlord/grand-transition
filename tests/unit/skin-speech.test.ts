import { gameCatalog, characterSkins } from '../../src/game-content.ts';
import { afterEach, describe, expect, test, vi } from 'vitest';
import { skinSpeechProfile } from '../../src/audio/skin-speech-profile.ts';
import { CharacterSpeech } from '../../src/audio/character-speech.ts';
import {
  MicrosoftRobotSpeech,
  selectMicrosoftRobotVoice,
} from '../../src/audio/microsoft-robot-speech.ts';
import type { SpeechRequest } from '../../src/audio/speech-port.ts';
import { GameSpeech } from '../../src/audio/game-speech.ts';
import { defaultSettings } from '../../src/persistence/codecs/settings-codec.ts';
import { publicPlayer } from '../fixtures/narration.ts';

const character = (id: string, species: 'human' | 'robot' = 'human') => ({
  id,
  species,
  voiceProfile: {
    ...gameCatalog.characters.find((value) => value.id === id)!.voiceProfile,
    pitch: 0.9,
  },
});
const femaleSpeechSkins = [
  'luxury-minister',
  'midnight-sensationalist--alternate',
  'oat-milk-reformist--alternate',
  'red-folded-chairman--alternate',
  'retiring-cassandra--statesman',
  'thunder-tribune--alternate',
  'government-ai--schoolteacher',
];
const voice = (name: string, local = true, lang = 'en-US') =>
  ({
    name: `Microsoft ${name} - English`,
    voiceURI: `local:${name}`,
    localService: local,
    lang,
    default: false,
...
describe('skin voice assignments', () => {
  test('GPU mode selects George and Emma while Government AI retains its native provider', () => {
    expect(
      skinSpeechProfile(character('red-folded-chairman'), 'default', 'gpu', 'en'),
    ).toMatchObject({ provider: 'neural', voiceUri: 'kokoro:bm_george', pitch: 0.9 });
    expect(skinSpeechProfile(character('luxury-minister'), 'default', 'gpu', 'en')).toMatchObject({
      provider: 'neural',
      voiceUri: 'kokoro:bf_emma',
    });
    expect(
      skinSpeechProfile(character('government-ai', 'robot'), 'schoolteacher', 'gpu', 'en'),
    ).toMatchObject({
  ...
  test('the robot roster exposes David, Mark, and schoolteacher Zira in skin order', () => {
    expect(characterSkins['government-ai']?.map(({ id }) => id)).toEqual([
      'default',
      'alternate',
      'schoolteacher',
    ]);
  ...
  test.each(femaleSpeechSkins)('%s uses the female British neural fallback', (id) => {
    ...
    expect(skinSpeechProfile(character(ownerId!), skinId, 'piper', 'en')).toMatchObject({
  ...
  test('male defaults and male alternate skins use George', () => {
    for (const [id, skin] of [
      ['red-folded-chairman', 'default'],
      ['velvet-mogul', 'silk-diplomat'],
    ]) {
      expect(skinSpeechProfile(character(id!), skin!, 'piper', 'en')).toEqual({
        provider: 'neural',
  ...
  test('Romanian robot skins use the Romanian neural voices, never the English-only exception', () => {
    for (const skin of ['default', 'alternate', 'schoolteacher']) {
      const profile = skinSpeechProfile(
        character('government-ai', 'robot'),
        skin,
        'piper',
        'ro-RO',
      );
  ...
  test('only the game locale selects the voice; an English match keeps English speech', () => {
    expect(
      skinSpeechProfile(character('red-folded-chairman'), 'default', 'piper', 'en'),
    ).toMatchObject({
      voiceUri: 'piper:vctk-p226',
      language: 'en-GB',
    });
  ...
  test.each([
    ['default', 'David', 'vctk-p226'],
    ['alternate', 'Mark', 'vctk-p226'],
    ['schoolteacher', 'Zira', 'vctk-p225'],
  ] as const)(
    'robot skin %s selects %s and retains the appropriate neural fallback',
    (skin, name, fallback) => {
      expect(
        skinSpeechProfile(character('government-ai', 'robot'), skin, 'piper', 'en'),
      ).toMatchObject({
        provider: 'microsoft-local',
        microsoftVoice: name,
        voiceUri: `piper:${fallback}`,
      });
    },
  );
});
...
function nativeHarness(voices = [voice('David'), voice('Mark'), voice('Zira')]) {
  ...
  const port = new MicrosoftRobotSpeech({
    service: () => service as unknown as SpeechSynthesis,
...
describe('local Microsoft robot speech', () => {
  ...
  test('a complete insult uses one utterance and native word boundaries drive phrase scores', () => {
    ...
    expect(h.events.onSegment).toHaveBeenLastCalledWith(0);
  ...
  test('Pause holds a completed delivery until resume and cancellation releases platform pause', () => {
    ...
    h.port.cancel();
    oldStart();
    expect(h.events.onStart).not.toHaveBeenCalled();
    expect(h.events.onEnd).toHaveBeenCalledOnce();
    expect(h.service.cancel).toHaveBeenCalledOnce();
    expect(h.service.resume).toHaveBeenCalledTimes(2);
  });
  test('cancellation from a start callback cannot emit a later score marker', () => {
    const h = nativeHarness();
    h.port.speak({ ...h.request, onStart: () => h.port.cancel() });
    h.fire(h.utterances[0]!, 'onstart');
    expect(h.events.onSegment).not.toHaveBeenCalled();
    expect(h.service.cancel).toHaveBeenCalledOnce();
  });
  test('missing native word boundaries finish once without estimated segment timers', () => {
    ...
    h.port.speak(h.request);
    h.fire(h.utterances[0]!, 'onstart');
    const ended = h.utterances[0]!.onend as () => void;
    ended();
    ended();
  ...
  test('native word progress keeps a long continuous delivery alive', () => {
    ...
    h.utterances[0]!.onboundary!.call(h.utterances[0]!, {
      name: 'word',
      charIndex: 12,
  ...
  test('a missing exact voice declines without speaking; a stalled utterance has a bounded, pause-aware failure', () => {
    ...
    expect(h.events.onError).toHaveBeenCalledOnce();
  });
});
...
test('human speech never uses the system service; robots fall back to neural when the exact voice is missing', () => {
  ...
  router.speak({ ...h.request, provider: 'neural' });
  expect(h.service.speak).not.toHaveBeenCalled();
  router.speak({
    ...h.request,
    provider: 'microsoft-local',
    microsoftVoice: 'Mark',
  ...
  expect(neural.speak).toHaveBeenCalledTimes(2);
  expect(h.service.speak).not.toHaveBeenCalled();
  router.speak({ ...h.request, provider: 'microsoft-local' });
  expect(h.service.speak).toHaveBeenCalledOnce();
  router.cancel();
});

test.each(['throw', 'error-event'] as const)(
  'native synchronous %s preserves neural fallback delivery callbacks',
  (failure) => {
    const h = nativeHarness();
    ...
    const neural = {
      available: true,
      cancel: vi.fn(),
      speak: vi.fn((request: SpeechRequest) => {
    ...
    expect(
      game.deliver(
        publicPlayer,
        { ...defaultSettings, speechEnabled: true },
        skinSpeechProfile(character('government-ai', 'robot'), 'default', 'piper', 'en'),
        h.events,
      ),
    ).toBe(true);
    expect(requests).toHaveLength(1);
    expect(requests[0]).toMatchObject({
      text: publicPlayer.insultText,
      voiceUri: 'piper:vctk-p226',
    });
