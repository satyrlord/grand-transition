...
const state = vi.hoisted(() => ({
  ...
  gpu: true,
  warmupFailure: false,
  logLevel: 'warning' as 'warning' | 'error',
  sessionOptions: undefined as unknown,
  runOptions: [] as unknown[],
  inputs: [] as Array<{
    input_ids: { data: BigInt64Array };
    pitch: { data: Float32Array };
    speed: { data: Float32Array };
  }>,
}));
vi.mock('../../src/audio/kokoro-gpu-manifest.json', () => ({ default: state.manifest }));
vi.mock('onnxruntime-web/ort-wasm-simd-threaded.asyncify.mjs?url&no-inline', () => ({
  default: '/grand-transition/tts/kokoro-gpu/ort-wasm-simd-threaded.asyncify.mjs',
}));
...
vi.mock('onnxruntime-web/webgpu', () => ({
  env: {
    ...
    set logLevel(value: 'warning' | 'error') {
      state.logLevel = value;
    },
    webgpu: {
      get device() {
        return state.gpu ? { queue: {} } : undefined;
      },
    },
  },
  Tensor: class {
  ...
  InferenceSession: {
    create: async (_model: unknown, options: unknown) => {
      ...
      return {
        run: async (input: (typeof state.inputs)[number], options: unknown) => {
          state.runOptions.push(options);
          state.inputs.push(input);
          if (state.warmupFailure) throw new Error('Internal private failure details');
          return {
            waveform: {
              data: new Float32Array(input.input_ids.data.length * 600).fill(0.1),
              dispose() {},
            },
            '/encoder/Clip_output_0': {
              data: new Float32Array(input.input_ids.data.length).fill(1),
              dispose() {},
            },
          };
        },
...
async function harness() {
  ...
  const files = new Map<string, Uint8Array>([
    ...
    [
      ...
      new TextEncoder().encode(
        JSON.stringify(
          Object.fromEntries(
            Array.from(' abcdefghijklmnopqrstuvwxyz.!').map((letter, index) => [letter, index + 1]),
          ),
        ),
      ),
    ],
    ['bm_george.bin', new Uint8Array(510 * 256 * 4)],
    ['bf_emma.bin', new Uint8Array(510 * 256 * 4)],
    ['ort-wasm-simd-threaded.asyncify.mjs', new Uint8Array([4])],
    ['ort-wasm-simd-threaded.asyncify.wasm', new Uint8Array([5])],
  ]);
  const manifest = {
    modelBytes: 3,
    modelSha256: hash(files.get('model-01.bin')!),
    shards: ['model-01.bin'],
    runtimeModule: {
  ...
  await import('../../src/audio/kokoro-gpu-worker.ts');
...
test('requires explicit loading, discards warmup PCM, and preserves pitch, tokens, and duration markers', async () => {
  ...
  expect(state.inputs.reduce((sum, input) => sum + input.input_ids.data.length - 2, 0)).toBe(
...
test.each(['missing-gpu', 'cpu-session', 'warmup', 'corrupt-shard', 'remote-origin'])(
  'fails closed for %s without private diagnostics',
  async (mode) => {
    const h = await harness();
    ...
    h.send({
      type: 'load',
      baseUrl: mode === 'remote-origin' ? 'https://invalid.test/tts/kokoro-gpu/' : h.base,
    });
    await vi.waitFor(() => expect(h.messages).toContainEqual({ type: 'error', id: null }));
    expect(
      h.messages.some((message) => message.type === 'ready' || message.type === 'speech'),
    ).toBe(false);
