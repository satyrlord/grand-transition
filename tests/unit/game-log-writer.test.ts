...
import { validateDevelopmentLog } from '../../tools/development-log-schema.ts';
import {
  maximumGameLogBytes,
  maximumStoredGameLogs,
  writeGameLog,
} from '../../tools/game-log-writer.ts';
...
const records = [
  {
    ...
    sceneId: 'studio',
    players: [
      { playerId: 'player-one', characterId: 'first' },
      { playerId: 'player-two', characterId: 'second' },
    ],
  },
  {
    type: 'action',
    sequence: 1,
    command: { type: 'resolve-round', source: 'user', payload: {} },
    move: { type: 'resolve-round' },
    outcome: 'accepted',
    errorCode: null,
    state: publicState,
  },
...
describe('game log writer', () => {
  test('accepts bounded terminal speech diagnostics and rejects arbitrary error or text fields', () => {
    ...
    const log = (speechDiagnostics: unknown) =>
      [...records.slice(0, -1), { ...records.at(-1), speechDiagnostics }]
        .map((record) => JSON.stringify(record))
        .join('\n');
    expect(validateDevelopmentLog(log(diagnostics))).toEqual({ seed: 73 });
    expect(() =>
      validateDevelopmentLog(
        log({ ...diagnostics, events: [{ ...diagnostics.events[0], text: 'hidden' }] }),
      ),
    ).toThrow('completion');
  });
  test('writes collision-safe log files inside the ignored repository folder', async () => {
    ...
    expect(await readFile(path.join(repositoryRoot, first), 'utf8')).toBe(validLog);
  });

  test('rejects invalid, empty, oversized, and out-of-repository writes', async () => {
    const repositoryRoot = await mkdtemp(path.join(os.tmpdir(), 'gt-log-'));
    await expect(writeGameLog({ text: '', repositoryRoot })).rejects.toThrow('empty');
    await expect(writeGameLog({ text: '{"type":"wrong"}\n', repositoryRoot })).rejects.toThrow(
      'header',
    );
    await expect(
      writeGameLog({
  ...
  test('rejects a log directory symlink that resolves outside the repository', async () => {
    const repositoryRoot = await mkdtemp(path.join(os.tmpdir(), 'gt-log-'));
    const outsideDirectory = await mkdtemp(path.join(os.tmpdir(), 'gt-outside-'));
  ...
  test('retains only the newest 50 match files and preserves unrelated logs', async () => {
    ...
    expect(files.filter((file) => file.startsWith('match-'))).toHaveLength(maximumStoredGameLogs);
...
test.each([
  (data: typeof records) => {
    data[0]!.players = undefined;
  },
  (data: typeof records) => {
    data[1]!.sequence = 2;
  },
  (data: typeof records) => {
    data[2]!.winner = 'unknown-player';
  },
  (data: typeof records) => {
    (data[1]!.state!.players['player-one'] as unknown as Record<string, unknown>).privateCardId =
      'secret';
  },
  (data: typeof records) => {
    data.pop();
