...
import { piperControls } from '../../src/audio/piper-text.ts';

test.each([0.75, 1, 1.5])(
  'Piper playback factor %s changes frequency while duration compensation retains tempo',
  async (pitch) => {
    ...
    for (let index = 0; index < input.length; index++)
      input[index] = 0.5 * Math.sin((2 * Math.PI * 440 * index) / 22050);
    ...
    let crossings = 0;
    for (let index = 4410; index < 26460; index++)
