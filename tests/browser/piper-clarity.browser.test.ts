...
import { createPiperClarity } from '../../src/audio/piper-clarity.ts';

test.each([44100, 48000])(
  'Piper clarity curve cuts bass and retains presence with headroom at %s Hz',
  (sampleRate) => {
    ...
    const highpass = new Float32Array(frequencies.length);
    const shelf = new Float32Array(frequencies.length);
    ...
    const db = Array.from(
      highpass,
      (gain, index) => 20 * Math.log10(gain * shelf[index]! * filter.headroom.gain.value),
    );
    expect(db[0]).toBeLessThan(-21);
    expect(db[1]).toBeLessThan(-14);
...
test('Piper filtering preserves rendered length, silence, and signal headroom', async () => {
  ...
  for (let i = 0; i < samples.length; i++)
    samples[i] =
      0.65 * Math.sin((2 * Math.PI * 75 * i) / rate) +
      0.2 * Math.sin((2 * Math.PI * 2500 * i) / rate);
  ...
  expect(output.length).toBe(rate * 2);
