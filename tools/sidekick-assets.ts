...
const layout: Record<string, { height: number; bottom: number }> = {};
...
if (command === 'build') {
  ...
  JSON.stringify(JSON.parse(await readFile(layoutPath, 'utf8'))) !== JSON.stringify(layout)
) {
  throw new Error('Sidekick alpha bounds are stale. Run node tools/sidekick-assets.ts build.');
}
