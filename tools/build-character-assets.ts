...
export const CHARACTER_VARIANT_FORMATS: readonly CharacterVariantFormat[] = Object.freeze([
  'avif',
  'webp',
]);
...
export async function readCharacterLayout(
  ...
  const layout = JSON.parse(
    await readFile(path.join(characterRoot, 'portrait-layout.json'), 'utf8'),
  );
  const ids = masterNames.map(assetId);
  if (
    layout.schemaVersion !== 1 ||
    !layout.portraits ||
    Array.isArray(layout.portraits) ||
    Object.keys(layout.portraits).length !== ids.length ||
    Object.keys(layout.portraits).some((id) => !ids.includes(id))
  ) {
  ...
  for (const id of ids) {
    ...
    if (
      !entry ||
      !['left', 'right'].includes(entry.facing) ||
  ...
  if (nativeAlpha) {
    ...
    for (let y = 0; y < info.height; y += 1) {
      for (let x = 0; x < info.width; x += 1) {
        ...
        if (data[offset] > 8)
          throw new Error(
            'Native character resize reaches the outer border. Increase the source safe margin.',
          );
        data[offset] = 0;
      }
    }
...
async function verifiedCachedAssets(
  ...
  for (const master of masters) {
    ...
    if (
      ...
      asset.ownerId !== ownerId(master.id) ||
      asset.skinId !== skinId(master.id) ||
      asset.facing !== layout[master.id].facing ||
      ...
      throw new Error(
        `Cached character source "${master.id}" changed or has invalid metadata; select it for rebuilding.`,
      );
    }
    ...
    for (const requirement of expected) {
      ...
      if (
        ...
        variant.bytes <= 0 ||
        variant.bytes > CHARACTER_BYTE_BUDGETS[requirement.format] ||
      ...
      try {
        ...
        throw new Error(
          `Cached character variant "${requirement.path}" failed byte, hash, dimension, or format validation.`,
          { cause: error },
        );
      }
      if (
...
export async function buildCharacterAssets({
  ...
  const cached = selected
    ? await verifiedCachedAssets(resolvedRoot, masters, selected, layout)
    : new Map<string, CharacterAsset>();
  const temporaryRoot = await mkdtemp(path.join(resolvedRoot, '.character-assets-build-'));
  ...
  try {
    const assets = await mapWithConcurrency(
      ...
      async ({ fileName, id, input }): Promise<CharacterAsset> => {
        ...
        if (cachedAsset) {
          await Promise.all(
            cachedAsset.variants.map((variant) =>
              copyFile(
                path.join(resolvedRoot, variant.path),
                path.join(temporaryRoot, variant.path),
              ),
            ),
          );
          return cachedAsset;
        ...
        const variants = await Promise.all(
          CHARACTER_VARIANT_SIZES.flatMap((width) =>
            CHARACTER_VARIANT_FORMATS.map(async (format) => {
              const { output, quality, lossless } = await encodeVariantWithMetadata(
                input,
                width,
                format,
              );
    ...
    await writeFile(
      path.join(temporaryRoot, 'character-manifest.json'),
    ...
    await rename(
      path.join(temporaryRoot, 'character-manifest.json'),
      path.join(resolvedRoot, 'character-manifest.json'),
    );
    return manifest;
  } finally {
...
if (invokedScript === path.resolve(fileURLToPath(import.meta.url))) {
  ...
  Promise.resolve()
    .then(() => {
      if (!validOptions)
        throw new Error('Use build-character-assets.ts [character-root] [--only id1,id2].');
      return buildCharacterAssets({ characterRoot, only });
    })
    .then((manifest) =>
      process.stdout.write(
        `Built ${manifest.assets.length} character masters and ${manifest.assets.length * CHARACTER_VARIANT_SIZES.length * CHARACTER_VARIANT_FORMATS.length} variants.\n`,
      ),
    )
    .catch((error) => {
