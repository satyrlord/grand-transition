...
const resolution = z
  .object({
    ...
    players: z.record(
      ...
      z
        .object({
          constructionText: z.string(),
          constructionStatus: status,
          phraseIds: z.array(id),
          grammarMistakes: count,
          selfDamage: count,
          outgoingDamage: count,
          prideAfter: count.max(100),
        })
        .strict(),
    ),
  })
...
const move = z.union([
  ...
  z
    .object({
      type: z.literal('select-phrase'),
      actorId: id.optional(),
      source: z.enum(['shared', 'private']),
      cardId: id.optional(),
      phraseId: id.nullable().optional(),
...
const headerSchema = z
  .object({
    type: z.literal('match-log'),
    formatVersion: z.literal(1),
    seed: count.max(0xffff_ffff),
...
const completionSchema = z
  .object({
    type: z.literal('match-complete'),
    winner: id,
    roundCount: count.min(1),
    state,
...
export function validateDevelopmentLog(text: string): { seed: number } {
  ...
  const validState = (value: z.infer<typeof state>): boolean =>
    players.has(value.activePlayerId) &&
    samePlayers(value.players) &&
    Object.entries(value.players).every(([key, value]) => players.get(key) === value.characterId) &&
    (value.latestResolution === null ||
      (players.has(value.latestResolution.openingPlayerId) &&
        samePlayers(value.latestResolution.players)));
  ...
  for (let index = 1; index < lines.length - 1; index += 1) {
    ...
    if (value.command.type === 'select-phrase' && value.move.type === 'select-phrase') {
      ...
      if (
        value.move.actorId !== value.command.actorId ||
        value.move.source !== card.source ||
        (redacted
          ? card.cardId !== undefined ||
            value.move.cardId !== undefined ||
            value.move.phraseId !== undefined ||
            value.move.text !== undefined
          : !card.cardId ||
            value.move.cardId !== card.cardId ||
            value.move.phraseId === undefined ||
            value.move.text === undefined ||
            (value.outcome === 'accepted' &&
              (value.move.phraseId === null || value.move.text === null)))
      ) {
  ...
  if (
    !completion.success ||
    !players.has(completion.data.winner) ||
    completion.data.state.phase !== 'results' ||
    !validState(completion.data.state) ||
