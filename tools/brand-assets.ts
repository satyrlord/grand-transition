...
const definitions = [
  {
    id: 'grand-transition-emblem',
    width: 1254,
    height: 1254,
    runtimeWidth: 640,
    runtimeHeight: 640,
    stem: 'grand-transition-emblem-640',
  },
  {
    id: 'title-proscenium-background',
    width: 1672,
    height: 941,
    runtimeWidth: 1672,
    runtimeHeight: 941,
    stem: 'title-proscenium-background',
  },
  {
    id: 'politburo-portrait-frame',
    width: 1086,
    height: 1448,
    runtimeWidth: 1086,
    runtimeHeight: 1448,
    stem: 'politburo-portrait-frame',
  },
];
...
export function validateBrandManifest(manifest: BrandManifest): BrandManifest {
  requireFact(
    manifest?.schemaVersion === 1 &&
      ...
      manifest.assets.length === definitions.length,
  ...
  for (const asset of manifest.assets) {
    ...
    requireFact(
      definition && !seen.has(asset.id),
      `Brand asset ${asset.id}: unknown or duplicate ID.`,
    );
    seen.add(asset.id);
    requireFact(
      asset.ownerType === 'interface' &&
        asset.ownerId === asset.id &&
        asset.sourceDescription === sourceDescription &&
        asset.licenseIdentifier === licenseIdentifier,
      `${asset.id}: invalid owner, source description, or license.`,
    );
    requireFact(
      JSON.stringify(asset.crop) === JSON.stringify(crop) &&
        JSON.stringify(asset.focalPoint) === JSON.stringify(focalPoint),
      `${asset.id}: invalid crop or focal point.`,
    );
    const expected = [
      {
        path: definition.id + '.png',
        width: definition.width,
        height: definition.height,
        format: 'png',
      },
      ...formats.map((format) => ({
        path: definition.stem + '.' + format,
        width: definition.runtimeWidth,
        height: definition.runtimeHeight,
        format,
      })),
    ];
    requireFact(
      Array.isArray(asset.variants) && asset.variants.length === 2,
      `${asset.id}: two runtime variants are required.`,
    );
    ...
    for (let index = 0; index < expected.length; index++) {
      ...
      requireFact(
        file &&
          Object.entries(expected[index]!).every(
            ([key, value]) => (file as Record<string, unknown>)[key] === value,
          ),
        `${asset.id}: invalid file path, dimensions, or format.`,
      );
      requireFact(
        /^[a-f0-9]{64}$/u.test(file.sha256) && Number.isSafeInteger(file.bytes) && file.bytes > 0,
        `${file.path}: invalid hash or bytes.`,
      );
      if (index > 0)
        requireFact(
          file.bytes <= (file.format === 'avif' ? 350 : 500) * 1024,
          `${file.path}: exceeds byte budget.`,
        );
    }
  }
  for (const format of formats) {
    const bytes = manifest.assets
      ...
      .reduce(
        (total, asset) =>
          total + asset.variants.find((variant) => variant.format === format)!.bytes,
        0,
      );
...
export async function buildBrandAssets(root = path.resolve('src/assets/brand')) {
  ...
  for (const definition of definitions) {
    ...
    for (const format of formats) {
      const pipeline = sharp(input).resize(definition.runtimeWidth, definition.runtimeHeight, {
        kernel: sharp.kernel.lanczos3,
      });
      const bytes = await (
        format === 'avif'
          ? pipeline.avif({ quality: 65, effort: 6 })
          : pipeline.webp({ quality: 78, alphaQuality: 100, effort: 6, smartSubsample: true })
      ).toBuffer();
...
export async function validateBrandAssets(root = path.resolve('src/assets/brand')) {
  const manifest = validateBrandManifest(
    JSON.parse(await readFile(path.join(root, 'brand-manifest.json'), 'utf8')),
  );
  for (const asset of manifest.assets) {
    for (const file of [asset.source, ...asset.variants]) {
      ...
      requireFact(
        hash(bytes) === file.sha256 && bytes.length === file.bytes,
        `${file.path}: hash or byte size mismatch.`,
      );
      requireFact(
        format === file.format && metadata.width === file.width && metadata.height === file.height,
        `${file.path}: decoded format or dimensions mismatch.`,
      );
    }
  }
  return manifest;
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  operation()
    .then(() => console.log(`Brand asset ${command} passed: three masters and six variants.`))
    .catch((error) => {
