...
const identity = {
  schemaVersion: 1,
  model: 'Piper-en_GB-vctk-medium',
  revision,
  source: 'https://huggingface.co/rhasspy/piper-voices',
  license: 'MIT; VCTK dataset CC BY 4.0',
  runtime: 'onnxruntime-web@1.29.0',
  pronunciation: 'phonemizer@1.2.1',
  sampleRate: 22050,
  samplesPerDurationFrame: 256,
  modification:
    'Expose predicted phoneme durations; retain all trained weights. Playback pitch uses compensated model durations.',
  voices: [
    { id: 'vctk-p226', name: 'Piper British male', lang: 'en-GB', speakerId: 95, default: true },
    {
      id: 'vctk-p225',
      name: 'Piper British female',
      lang: 'en-GB',
      speakerId: 107,
      default: false,
    },
...
const sources = [
  ...
  [
    ...
    '7f85e6391ed0f7f46e4abd19345929a16be931a0c9945086f96692dce2087fa8',
  ],
...
const expectedFiles = [
  {
    path: 'model.onnx',
    bytes: 76952867,
    sha256: '2e141b1a63071d1f257d90939d849569b05a476a96ed5b9def69b654f5868f96',
  },
  {
    path: 'config.json',
    bytes: 6637,
  ...
  },
  {
    path: 'MODEL_CARD',
    bytes: 326,
    sha256: '53ec6b1fd90d9125d5bebac55261efd27f7e8d6dc962f605a5b5f796df2ec7fb',
  },
  {
    path: 'NOTICE.txt',
    bytes: 765,
    sha256: '63b8785b0c36155273608df5abaa7901c7ff5fcaf42286a778d1de3b3f7d2ab5',
  },
  {
    path: 'piper-LICENSE',
    bytes: 1071,
  ...
  },
  {
    path: 'VCTK-LICENSE',
    bytes: 17416,
    sha256: 'b34e17103bfb246f2549fc82a279e6ba28834e0cb42f76a92efc14b72e3a3723',
  },
  {
...
async function build() {
  ...
  await mkdir('tmp/piper-source', { recursive: true });
  const assets = [];
  for (const [name, expected] of sources) {
    ...
    try {
      ...
      bytes = await download(source + name);
  ...
  let schema;
  try {
    schema = await readFile('tmp/piper-source/onnx.proto');
  } catch {
    schema = await download(schemaSource.url);
    await writeFile('tmp/piper-source/onnx.proto', schema);
  }
  ...
  const model = Model.decode(assets[0]!) as unknown as {
    graph: {
  ...
  if (
    ...
    throw new Error('The pinned duration tensor is missing.');
  ...
  model.graph.output.push({
    ...
    type: {
      tensorType: {
        elemType: 1,
        shape: { dim: [{ dimValue: 1 }, { dimValue: 1 }, { dimParam: 'phonemes' }] },
      },
    },
  });
  await writeFile(path.join(root, 'model.onnx'), Model.encode(model).finish());
  await writeFile(path.join(root, 'config.json'), assets[1]);
  await writeFile(path.join(root, 'MODEL_CARD'), assets[2]);
  await writeFile(
  ...
  for (const [from, to] of [
    ['LICENSE', 'onnx-runtime-LICENSE'],
    ['ThirdPartyNotices.txt', 'onnx-runtime-NOTICES'],
  ]) {
    await writeFile(
      path.join(root, to),
      await download(`https://raw.githubusercontent.com/microsoft/onnxruntime/v1.29.0/${from}`),
    );
  }
  await writeFile(
    path.join(root, 'piper-LICENSE'),
    await download(
      'https://raw.githubusercontent.com/rhasspy/piper/73c04d81d5590ecc46e522de3601ce7fb29fc2be/LICENSE.md',
    ),
  );
  await writeFile(
    path.join(root, 'VCTK-LICENSE'),
    await download(
      'https://datashare.ed.ac.uk/server/api/core/bitstreams/956a1688-0b59-428c-8a2f-10837433dde3/content',
    ),
  );
  const files = [];
  ...
  if (!isDeepStrictEqual(files, expectedFiles))
    throw new Error('The generated Piper outputs do not match the pinned package.');
  await writeFile(
    path.join(root, 'manifest.json'),
    JSON.stringify({ ...identity, sources: manifestSources, files: expectedFiles }, null, 2) + '\n',
  );
...
export async function validateNeuralAssets(assetRoot = root) {
  ...
  if (
    !isDeepStrictEqual(manifest, { ...identity, sources: manifestSources, files: expectedFiles })
  ) {
  ...
  for (const file of expectedFiles) {
    ...
    if (bytes.length !== file.bytes || hash(bytes) !== file.sha256)
      throw new Error('Neural asset hash mismatch.');
  }
  if ((await stat(path.join(assetRoot, 'model.onnx'))).size >= 100 * 1024 * 1024)
    throw new Error('The neural model exceeds the single-file budget.');
  const config = JSON.parse(await readFile(path.join(assetRoot, 'config.json'), 'utf8'));
  if (
    config.audio.sample_rate !== 22050 ||
    config.speaker_id_map.p226 !== 95 ||
    config.speaker_id_map.p225 !== 107
  )
    throw new Error('The Piper voice configuration is invalid.');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  else throw new Error('Use neural-speech-assets.ts build or validate.');
}
