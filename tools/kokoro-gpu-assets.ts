...
const identity = {
  ...
  pronunciation: 'phonemizer@1.2.1',
  sampleRate: 24000,
...
async function pinnedInput(cache: string, url: string, expected: string): Promise<Buffer> {
  ...
  try {
    ...
    if (hash(bytes) !== expected)
      throw new Error('The pinned GPU speech source hash does not match.');
...
async function build() {
  const sourceBytes = await pinnedInput(
    'tmp/tts-comparison-2026-09-09/assets/kokoro-fp32-original.onnx',
    `${source}/resolve/${revision}/onnx/model.onnx`,
    sourceSha256,
  );
  const schema = await pinnedInput(
    'tmp/tts-comparison-2026-09-09/assets/onnx.proto',
    'https://raw.githubusercontent.com/onnx/onnx/v1.17.0/onnx/onnx.proto',
    schemaSha256,
  );
  const Model = protobuf.parse(schema.toString()).root.lookupType('onnx.ModelProto');
  // Only the graph fields that the pitch input and duration output change are typed here.
  const model = Model.decode(sourceBytes) as unknown as {
    graph: {
  ...
  if (producer < 0 || !model.graph.node.some((node) => node.output.includes(duration)))
    throw new Error('The pinned GPU graph is invalid.');
  for (const node of model.graph.node)
    node.input = node.input.map((name) => (name === pitch ? 'grand_transition_f0' : name));
  model.graph.node.splice(producer + 1, 0, {
    name: 'grand_transition_pitch',
    opType: 'Mul',
    input: [pitch, 'pitch'],
    output: ['grand_transition_f0'],
  });
  model.graph.input.push({
    name: 'pitch',
  ...
  model.graph.output.push({
    ...
    type: {
      tensorType: {
        elemType: 1,
        shape: { dim: [{ dimValue: 1 }, { dimParam: 'sequence_length' }] },
      },
    },
  });
  const derived = Model.encode(model).finish();
  if (derived.length !== identity.modelBytes || hash(derived) !== modelSha256)
    throw new Error('The derived GPU model hash does not match.');
  await mkdir(root, { recursive: true });
  for (const [index, name] of shards.entries())
    await writeFile(
      path.join(root, name),
      derived.subarray(index * 96 * 1024 * 1024, (index + 1) * 96 * 1024 * 1024),
    );
  ...
  for (const name of ['ort-wasm-simd-threaded.asyncify.wasm']) {
    await writeFile(
      path.join(root, name),
      await readFile(path.join('node_modules/onnxruntime-web/dist', name)),
    );
  }
  await writeFile(
    path.join(root, 'phonemizer-LICENSE'),
    await readFile('node_modules/phonemizer/LICENSE'),
  );
  for (const [name, url] of [
    ...['bm_george', 'bf_emma'].map((id) => [
      `${id}.bin`,
      `${source}/resolve/${revision}/voices/${id}.bin`,
    ]),
    ...
    await pinnedInput(
      path.join(root, name!),
      url!,
      files.find((file) => file.path === name)!.sha256,
    );
  await writeFile(
  ...
  await writeFile(
    'src/audio/kokoro-gpu-manifest.json',
    JSON.stringify({ ...identity, files }, null, 2) + '\n',
  );
  await validateKokoroGpuAssets();
}
export async function validateKokoroGpuAssets(
  assetRoot = root,
  compiledManifestPath = 'src/audio/kokoro-gpu-manifest.json',
) {
  const manifest = JSON.parse(await readFile(path.join(assetRoot, 'manifest.json'), 'utf8'));
  if (!isDeepStrictEqual(manifest, { ...identity, files }))
    throw new Error('The GPU speech manifest or file inventory is invalid.');
  const compiled = JSON.parse(await readFile(compiledManifestPath, 'utf8'));
  if (!isDeepStrictEqual(compiled, manifest))
    throw new Error('The compiled GPU speech manifest is stale.');
  if (
    !isDeepStrictEqual(
      (await readdir(assetRoot)).sort(),
      [...files.map((file) => file.path), 'manifest.json'].sort(),
    )
  )
    throw new Error('The GPU speech directory has an invalid inventory.');
  const moduleBytes = await readFile(
    path.join('node_modules/onnxruntime-web/dist', identity.runtimeModule.path),
  );
  if (
    moduleBytes.length !== identity.runtimeModule.bytes ||
    hash(moduleBytes) !== identity.runtimeModule.sha256
  )
    throw new Error('The GPU runtime module hash does not match.');
  ...
  for (const file of files) {
    ...
    if (
      ...
      throw new Error('GPU speech asset hash or size mismatch.');
    if (shards.includes(file.path)) {
      modelHash.update(bytes);
      modelBytes += bytes.length;
    }
  }
  if (modelBytes !== identity.modelBytes || modelHash.digest('hex') !== modelSha256)
    throw new Error('The reassembled GPU model hash does not match.');
  console.log(
    `GPU speech asset validation passed (${files.reduce((sum, file) => sum + file.bytes, 0)} manifested bytes).`,
  );
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv[2] === 'build') await build();
  else if (process.argv[2] === 'validate') await validateKokoroGpuAssets();
  else throw new Error('Use kokoro-gpu-assets.ts build or validate.');
}
