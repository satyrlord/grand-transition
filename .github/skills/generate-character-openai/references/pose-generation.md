...
Preserve the accepted identity, style, proportions, construction, palette, and prop system.

## Resume without duplicate work

Check each required state in the status record.
If a locked pose exists, verify its hash and review.
...
Use exact 2048 by 2048 PNG output.
...
Keep all five poses different from selection.
Keep all five poses different from each other.

## Review one state before the next request

Run a dry run.
Verify the locked selection is the only reference.
Send one request after the dry run passes.
Inspect the raw output before preparation.

Apply only the approved background preparation.
Inspect the prepared output at source size.
Inspect light and dark composites.
Inspect the face or display.
Inspect all visible hands, appendages, props, and extremities.
...
Require six distinct poses.
Require at least five expressions.
Verify identity, scale, baseline, facing, contours, shading, and prop continuity.
...
Recheck the shipping tree before installation.
Preserve every unrelated source byte.

If the accepted package is already installed, verify all hashes.
