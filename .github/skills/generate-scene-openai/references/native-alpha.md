...
Examine the full image, source-scale contours, and composites against light and dark backgrounds.
...
It keeps all red, green, and blue (RGB) values and every other alpha value.
It keeps alpha-1 contour pixels within that distance.
...
Run the color check.
Complete a passing review that matches the prepared image hash.
Keep raw inspection and preparation evidence even when the prepared result passes.
For scenes, run the reviewed scene master preparation before the following metadata steps.
For characters, keep the exact 2048-square master without resizing.
...
Make sure that metadata operations keep the reviewed decoded pixels.
