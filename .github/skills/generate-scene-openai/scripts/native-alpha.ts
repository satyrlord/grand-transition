...
async function decode(bytes: Buffer): Promise<Raster> {
  ...
  if (info.channels !== 4)
    throw new Error('For native-alpha preparation, use decoded RGBA pixels.');
...
function nearInterior(data: Buffer, width: number, height: number, x: number, y: number) {
  ...
  for (let row = Math.max(0, y - radius); row <= Math.min(height - 1, y + radius); row += 1) {
    for (
      let column = Math.max(0, x - radius);
...
function inspectRaster({ data, width, height, hasAlpha }: Raster) {
  ...
  if (topology.partialAlphaPixels === 0)
    issues.push('The source must contain partial-alpha contour pixels.');
  ...
  if (topology.nontransparentBorderPixels > 0)
    issues.push('The outer border must be fully transparent.');
  if (contourRatio < NATIVE_ALPHA_MIN_CONTOUR_RATIO) {
    issues.push(
      `At least ${NATIVE_ALPHA_MIN_CONTOUR_RATIO * 100}% of partial alpha must be within ` +
  ...
  return {
    ...
    topology: {
      ...
      contourRatio,
      nearOpaqueRatio,
      detachedAlphaOnePixels,
      detachedStrongerAlphaPixels,
    },
  };
}
...
export async function prepareNativeAlpha(bytes: Buffer) {
  ...
  const output =
    clearedAlphaOnePixels === 0
      ? bytes
      : await sharp(data, { raw: { width: raster.width, height: raster.height, channels: 4 } })
          .png()
          .toBuffer();
  return {
    output,
