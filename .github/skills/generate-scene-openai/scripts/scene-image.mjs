...
async function requireUnusedPath(file) {
  ...
  throw new Error(
    'The output path is in use. Use a different output path for each run and its evidence.',
  );
}
...
function imageDimensions(size) {
  ...
  if (!Number.isSafeInteger(width * height) || width < 1 || height < 1)
    throw new Error('Use positive integer image dimensions.');
...
export async function readApiKey(repo = REPO) {
  if (git(['ls-files', '--', '.env.local'], repo))
    throw new Error('The environment file must be untracked.');
  try {
    git(['check-ignore', '-q', '--', '.env.local'], repo);
  } catch {
...
export async function inspectImage(bytes, expectedSize = NATIVE_SIZE) {
  ...
  if (
    ...
    throw new Error(
      `The image must be a ${expectedSize.width}x${expectedSize.height} PNG. The image is a ${metadata.width}x${metadata.height} ${metadata.format}.`,
    );
  }
  await sharp(bytes, { failOn: 'warning' }).raw().toBuffer();
  return {
...
async function main() {
  ...
  if (command === 'generate' && values.prompt && values.out) {
    ...
    const { promptText, referenceImages, ...inputs } = await inspectInputs(
      values.prompt,
      values.reference,
    );
    ...
    if (values['dry-run']) {
      console.log(
        JSON.stringify({
          ...plan,
          ...record,
          referenceCount: inputs.references.length,
    ...
    await writeJson(path.join(out, 'request-record.json'), {
      ...record,
    ...
    try {
      ...
      await status({
        ...
        code: 'dimensions-or-decode',
        automaticRetries: 0,
      });
      throw new Error(
        'The saved candidate failed dimension or decode checks. Before a new request, examine the saved candidate.',
      );
    }
    ...
    console.log(
      JSON.stringify({ prepared: out, ...record, visualReview: 'required before integration' }),
    );
  } else if (command === 'prepare' && values.input && values.review && values.scene && values.out) {
    const out = await temporaryPath(values.out);
    await requireUnusedPath(out);
    await requireUnusedPath(`${out}.preparation.json`);
    const { output, record } = await prepareImage(
    ...
    console.log(
      JSON.stringify({
        prepared: out,
        ...record.output,
        shippingApproval: 'Final alpha, provenance, and runtime checks must pass.',
      }),
    );
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    console.error(error instanceof Error ? error.message : 'The scene helper failed.');
    process.exitCode = 1;
  });
}
