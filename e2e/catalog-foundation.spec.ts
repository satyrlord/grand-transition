...
for (const viewport of gateViewports(viewports)) {
  test(`complete foundation selection at ${viewport.width}x${viewport.height}`, async ({
    ...
    await expect
      .poll(
        async () => {
          const windows = await page.locator('.roster-portrait-window').evaluateAll((elements) =>
            elements.map((element) => {
              const box = element.getBoundingClientRect();
              const inset = Math.max(3, Math.floor(Math.min(box.width, box.height) * 0.08));
              return {
                left: Math.ceil(box.left) + inset,
                top: Math.ceil(box.top) + inset,
    ...
    for (const side of ['one', 'two'] as const) {
      const selector = page.locator(
        side === 'one' ? '#playerOneCharacterId' : '#playerTwoCharacterId',
      );
      ...
      for (const character of catalog.characters) {
        ...
        await expect(stage.locator('.contestant-record strong')).toHaveText(
          locale.messages[character.nameKey]!,
        );
        await stage
          .locator('.contestant-portrait')
          .evaluate((image: HTMLImageElement) => image.decode());
        await expect(stage.locator('.contestant-portrait')).toHaveAttribute(
          'src',
          new RegExp(character.id, 'u'),
        );
        const geometry = await stage.evaluate((element) => {
          ...
          const weaknesses = element
            .querySelector('.contestant-record > span')!
          ...
          return {
            ...
            nameFits:
              nameBox.left >= record.left &&
              nameBox.right <= record.right &&
              nameBox.top >= record.top &&
              nameBox.bottom <= weaknesses.top + 1,
            nameInside:
              nameBox.left >= 0 &&
              nameBox.top >= 0 &&
              nameBox.right <= innerWidth &&
              nameBox.bottom <= innerHeight,
            portraitInside:
              portrait.left >= stageBox.left - 1 &&
              portrait.top >= stageBox.top - 1 &&
              portrait.right <= stageBox.right + 1 &&
              portrait.bottom <= stageBox.bottom + 1,
            pageFits:
              document.documentElement.scrollWidth === innerWidth &&
              document.documentElement.scrollHeight === innerHeight,
          };
        });
        expect(geometry, `${side}: ${character.id}`).toEqual({
          loaded: true,
          nameFits: true,
          nameInside: true,
          portraitInside: true,
    ...
    await page.screenshot({
      path: testInfo.outputPath(`catalog-${viewport.width}x${viewport.height}.png`),
    });
  });
}
