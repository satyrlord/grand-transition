    ...
    (settings) => {
      ...
      class ControlledWorker extends EventTarget {
        ...
        constructor(url: string | URL) {
          super();
          this.gpu = String(url).includes('kokoro-gpu-worker');
          if (this.gpu)
            target.finishGpu = (ready) =>
              this.emit(
                ready
                  ? {
                      type: 'ready',
...
test('main-menu GPU loading keeps setup available, fits supported sizes, and clears on readiness', async ({
  page,
}) => {
  await holdGpu(page);
  ...
  await page.screenshot({ path: '.impeccable/review/title-gpu-loading-forced-colors.png' });
  await page.evaluate(() =>
    (window as unknown as { finishGpu: (ready: boolean) => void }).finishGpu(true),
  );
...
test('GPU failure presents the fallback on the menu', async ({ page }) => {
  ...
  await expect(
    page.getByText('GPU voices are unavailable. Using local Piper voices.', { exact: true }),
  ).toBeVisible();
  await expect(page.getByRole('button', { name: 'Multiplayer' })).toBeEnabled();
});
