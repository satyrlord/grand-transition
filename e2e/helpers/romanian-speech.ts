...
export const romanianInterfaceText = {
  ...
  'ro-RO': {
    settings: 'Setări',
    close: 'Închide',
    multiplayer: 'Doi jucători',
    end: 'Gata',
    speech: 'Vorbire activată',
    start: 'Începe meciul',
  },
} as const;
...
async function chooseFighter(
  ...
  await page
    .locator(`.roster-choice[data-character-id="${character}"][data-skin-id="default"]`)
    .click();
...
export async function configureRomanianSpeech(
  ...
  await page.addInitScript(
    (initial) => {
      ...
      const evidence: RomanianSpeechEvidence = {
        native: [],
        neural: [],
        messages: [],
        timeline: [],
      ...
      if (heap) {
        ...
        setInterval(() => {
          evidence.peakHeapBytes = Math.max(evidence.peakHeapBytes, heap.usedJSHeapSize);
        }, 250);
      }
