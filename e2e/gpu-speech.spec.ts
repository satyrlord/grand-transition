...
test('development serves the pinned GPU runtime without JavaScript transforms', async ({
  request,
}) => {
  const response = await request.get(
    'http://127.0.0.1:5174/grand-transition/node_modules/onnxruntime-web/dist/ort-wasm-simd-threaded.asyncify.mjs',
  );
...
test('the development GPU worker boots without importing public assets as JavaScript', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(
    () =>
      new Promise<string>((resolve) => {
        const worker = new Worker('/grand-transition/src/audio/kokoro-gpu-worker.ts', {
          type: 'module',
        });
