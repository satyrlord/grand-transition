...
function summary(values: readonly number[]): {
  ...
  return {
    ...
    worstMs: Math.round(Math.max(...values)),
  };
}

test('records the Romanian speech workload on the least-capable supported environment', async ({
  page,
  browser,
}, info) => {
  ...
  const synthesized = evidence.timeline.filter(
    ({ direction, data }) => direction === 'in' && (data as { type?: string }).type === 'speech',
  );
  ...
  const progress = evidence.timeline.filter(
    ({ direction, data }) => direction === 'in' && (data as { type?: string }).type === 'progress',
  );
  const packagedBytes = await page.evaluate(async () => {
    const manifest = (await (
      await fetch(new URL('tts/ro/manifest.json', document.baseURI))
    ).json()) as { files: { bytes: number }[] };
  ...
  const coldDownload = progress.length
    ? {
        ...
        packagedBytes,
        reportedLoadedBytes: Math.max(
          ...progress.map(({ data }) => (data as { loaded: number }).loaded),
        ),
      }
    : null;
      ...
      () =>
        page.evaluate(
          () =>
            ...
            ).romanianSpeechEvidence.neural.length,
        ),
      { timeout: 120_000 },
    )
  ...
  const answers = afterCancel.timeline.filter(
    ({ direction, data }) => direction === 'in' && (data as { type?: string }).type === 'speech',
  );
  ...
  const perVoiceWarm = Object.fromEntries(
    [...new Set(samples.map(({ voiceId }) => voiceId))].map((voiceId) => [
      voiceId,
      summary(
        samples.filter(({ cold, voiceId: id }) => !cold && id === voiceId).map(({ ms }) => ms),
      ),
    ]),
  );
  ...
  const report = {
    ...
    perVoiceColdInitializationMs: Object.fromEntries(
      samples.filter(({ cold }) => cold).map(({ voiceId, ms }) => [voiceId, Math.round(ms)]),
    ),
    samples: samples.map(({ voiceId, cold, ms }) => ({ voiceId, cold, ms: Math.round(ms) })),
    warmSynthesis: summary(warm.map(({ ms }) => ms)),
    perVoiceWarm,
    perVoiceNote:
      'The first synthesis of each voice is cold. Warm samples are one per voice in a single run; the raw samples above carry the real evidence.',
    peakJavaScriptHeapBytes: evidence.peakHeapBytes,
    peakMemoryNote:
      'performance.memory reports the JavaScript heap only, quantised by Chromium, and excludes the WebAssembly heap that holds the weights (the shipped package is the figure below). The WebAssembly heap remains unverified; see coldDownload.packagedBytes.',
    cancellation,
    unverified: {
      device:
        'No low-end physical device or mobile browser was available; the throttled desktop Chromium run is the reproducible proxy.',
      wasmHeap: true,
    },
  };
  ...
  expect(
    evidence.messages.filter((message) => (message as { type?: string }).type === 'error'),
  ).toEqual([]);
  await writeFile(
    info.outputPath('romanian-speech-benchmark.json'),
