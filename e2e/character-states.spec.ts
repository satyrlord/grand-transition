...
for (const entry of stateManifest.packages) {
  test(`${entry.ownerId} ${entry.skinId} renders nine selected states without layout shift`, async ({
    page,
  }, testInfo) => {
    ...
    page.on('request', (request) => {
      if (allStateIds.some((id) => request.url().includes('/' + id + '-')))
    ...
    expect(
      await page.evaluate(() =>
        (window as unknown as { characterStateShifts: number[] }).characterStateShifts.reduce(
          (a, b) => a + b,
          0,
        ),
      ),
    ...
    await page
      .locator(`.roster-choice[data-character-id="${entry.ownerId}"][data-skin-id="default"]`)
      .click();
    ...
    await page
      .locator('grand-transition-character img')
    ...
    await page
      .locator('grand-transition-character')
      .first()
      .evaluate(async (element) => {
        const presenter = element as HTMLElement & {
          restState: string;
          updateComplete: Promise<unknown>;
        };
        presenter.restState = 'idle';
        await presenter.updateComplete;
      });
    const allowedIds = stateManifest.assets
      .filter(
        (asset) =>
    ...
    expect(
      stateRequests.every((url) => allowedIds.some((id) => url.includes('/' + id + '-'))),
    ).toBe(true);
    await page.evaluate(() => document.fonts.ready);
    ...
    await expect(player.locator('[data-state-id="selection"] img')).toHaveAttribute(
      'src',
      new RegExp('/' + selectedAssetId + '-960x960'),
    );
    await page
      .locator('.common-phrases, .sentence-ledger, .match-actions')
    ...
    for (const [index, state] of entry.states.entries()) {
      ...
      await page.locator('grand-transition-match').evaluate(
        async (element, cue) => {
          const match = element as HTMLElement & {
            snapshot: { revision: number; players: { portraitCue: unknown }[] };
            updateComplete: Promise<unknown>;
          };
          match.snapshot = {
            ...match.snapshot,
            revision: cue.sequence,
            players: match.snapshot.players.map((player, index) =>
              index === 0 ? { ...player, portraitCue: cue } : player,
            ),
          };
      ...
      if (['idle', 'heavy-hit', 'grammar-mistake'].includes(state.stateId)) {
        await page.screenshot({
          path: testInfo.outputPath(`${entry.ownerId}-${entry.skinId}-${state.stateId}.png`),
        });
      }
    }
    await page.evaluate(
    ...
    expect(
      await page.evaluate(
        () => (window as unknown as { characterStateShifts: number[] }).characterStateShifts,
      ),
    ).toEqual([]);
  });
}
