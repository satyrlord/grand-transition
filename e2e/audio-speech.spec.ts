...
type AudioEvidence = {
  starts: Array<{ loop: boolean; at: number; wall: number }>;
  stops: number[];
  ...
  await page.addInitScript(
    ({ unavailable, settings }) => {
      localStorage.setItem(
        ...
        JSON.stringify({ ...settings, turnTimerSeconds: null }),
      );
      ...
      class MeasuredContext extends (native ?? class {}) {
        ...
        override createGain() {
          ...
          if (evidence.gains.length === 0) {
            ...
            gain.connect = ((node: AudioNode) =>
              connect(
                node === this.destination ? evidence.analyser! : node,
              )) as GainNode['connect'];
          }
          ...
          source.start = (at = 0) => {
            ...
            if (timings && source.buffer?.sampleRate === 22050) {
              timings.push({
                ...
                at: performance.now(),
                seconds: source.buffer.duration / source.playbackRate.value,
              });
              source.addEventListener('ended', () => {
                timings.push({ event: 'playback-end', at: performance.now() });
              });
            }
            start(at);
          };
          source.stop = (at = 0) => {
            evidence.stops.push(at);
            stop(at);
          };
          return source;
        }
      ...
      Object.defineProperty(window, 'AudioContext', {
        configurable: true,
        value: unavailable || !native ? undefined : MeasuredContext,
      });
      Object.defineProperty(window, 'speechSynthesis', {
        configurable: true,
        value: {
          speak: () => {
            throw new Error('Legacy platform speech must not run.');
          },
        },
      });
    },
...
test('native decoded menu, scene, cues, mute, and exit under production CSP', async ({
  page,
}, info) => {
  ...
  page.on('request', (request) => {
    if (['xhr', 'websocket'].includes(request.resourceType()))
      errors.push(`Forbidden request: ${request.resourceType()}`);
    if (
    ...
    const result = await page.evaluate(async (id) => {
      ...
      const observedOnsetMs = await new Promise<number>((resolve, reject) => {
        const timeout = setTimeout(
          () => reject(new Error('The cue produced no audio signal.')),
          1000,
        );
  ...
  await expect
    .poll(() => page.evaluate(() => window.audioEvidence.starts.filter(({ loop }) => loop).length))
    .toBe(2);
  await expect.poll(() => samplePeak(page)).toBeGreaterThan(0.001);
  ...
  await expect
    .poll(() => page.evaluate(() => window.audioEvidence.contexts.map((context) => context.state)))
    .toEqual(['closed']);
  expect(errors).toEqual([]);
  await recordEvidence(page, info, 'audio-measurements', {
    cueTimes,
    menu: 'nonzero samples',
    scene: 'nonzero samples',
    mute: 'exact zero samples',
...
test('speech controls omit voice selection and fit supported landscape states', async ({
  page,
}, info) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await probe(page);
...
test('unavailable services keep the controls usable and the fallback silent', async ({
  page,
}, info) => {
  ...
  expect(
    await page.evaluate(() => ({
      sources: window.audioEvidence.starts.length,
      speech: window.audioEvidence.speech.length,
    })),
  ).toEqual({ sources: 0, speech: 0 });
  expect(errors).toEqual([]);
});

test('real local neural speech narrates both public bubbles before Victory', async ({
  page,
}, info) => {
    ...
    window.Worker = class extends native {
      ...
      override postMessage(
        ...
        if (command.type === 'synthesize')
          timings.push({ event: 'requested', at: performance.now(), id: command.id });
  ...
  await page.evaluate(() => {
    const app = document.querySelector('grand-transition-app') as unknown as {
      matchState: import('../src/engine/match-lifecycle.ts').MatchState;
    };
    const state = app.matchState;
    const id = state.playerOrder[0]!;
    app.matchState = {
      ...state,
      playerStates: { ...state.playerStates, [id]: { ...state.playerStates[id]!, pride: 1 } },
    };
  });
  await page.getByRole('button', { name: 'End', exact: true }).click();
  ...
  for (const [index, id] of ['player-two', 'player-one'].entries()) {
    ...
    await expect(page.locator('.match-screen')).toHaveAttribute('data-delivery-phase', 'reciting', {
      timeout: 90_000,
    });
    ...
    await expect(page.locator('.delivery-total strong')).toHaveText(
      String(result.players[id]!.outgoingDamage),
      { timeout: 90_000 },
    );
  ...
  await expect
    .poll(
      async () => JSON.parse((await storedHistory(page)).at(-1) ?? '{}').speechDiagnostics?.status,
    )
    .toBe('finished');
  const storedDiagnostics = JSON.parse((await storedHistory(page)).at(-1)!).speechDiagnostics;
  expect(storedDiagnostics.status).toBe('finished');
  expect(
    storedDiagnostics.events.filter((event: { type: string }) => event.type === 'playback-end'),
  ).toHaveLength(2);
  expect(
    storedDiagnostics.events
      .filter((event: { type: string }) => event.type === 'presentation-total')
      .map((event: { value: number }) => event.value),
  ).toEqual([
    result.players['player-two']!.outgoingDamage,
    result.players['player-one']!.outgoingDamage,
  ]);
  ...
  expect(commands.map((command) => command.segments!.join(''))).toEqual([
    result.players['player-two']!.insultText,
  ...
  expect(timings.filter(({ event }) => event === 'requested')[1]!.at).toBeLessThan(
    timings.find(({ event }) => event === 'playback-end')!.at,
  );
  await recordEvidence(page, info, 'neural-speech', {
    readyMs,
    synthesized: commands,
    timings,
    cache: 'fresh browser context; model initialized before the exchange',
    model: 'Piper VCTK medium',
    engine: 'local ONNX WASM',
    privateDraftRequests: 0,
    terminalOverlay: 'after both deliveries',
    physicalListening: 'not performed',
  });
  ...
  await expect(page.locator('.match-history-entry pre').first()).toContainText(
    '"speechDiagnostics"',
  );
