...
import { clearStoredHistory, storedHistory } from './helpers/stored-data.ts';
...
test('a hotseat match reaches persistent victory and restores title history', async ({
  ...
  const survivedCarries = new Set(
    plan.finalState.resolutionHistory.flatMap((round) =>
  ...
  for (const action of plan.actions) {
    ...
    if (await page.locator('.match-screen').getAttribute('data-delivery-phase')) {
      ...
      for (const playerId of [
        ...
        ...Object.keys(expectedResolution.players).filter((id) => id !== action.command.actorId),
      ]) {
        const result = expectedResolution.players[playerId]!;
        if (
          expectedResolution.players[playerId]!.selfDamage > 0 &&
          plan.finalState.resolutionHistory.at(-1) === expectedResolution
        )
          continue;
        if (!result.completeValidInsult) {
          ...
          await expect(scoreCard.locator('.delivery-status')).toHaveText('Hesitation');
          const playerName = await match.evaluate(
            (element, id) =>
              (
                element as unknown as {
                  snapshot: { players: Array<{ playerId: string; characterName: string }> };
                }
        ...
        const expectedComponents =
          (result.score?.breakdown.filter(
            (item) => item.kind === 'clause-score' || item.kind === 'finisher-bonus',
          ).length ?? 0) + Number(result.comebackBonus > 0);
        await expect(scoreCard.locator('.delivery-score')).toHaveCount(expectedComponents);
        ...
        if (result.weaknessActivated) {
          await expect(scoreCard.locator('.score-factor--weakness').first()).toHaveText('×2');
        }
        if (result.comebackBonus > 0) {
          await expect(scoreCard.locator('[data-score-kind="comeback"]')).toContainText(
      ...
      const nextSnapshot = await match.evaluate(
        (element) =>
          (
            element as unknown as {
              snapshot?: {
                phase: string;
                round: number;
                players: Array<{ characterName: string; pride: number }>;
              };
            }
          ).snapshot,
      );
      if (nextSnapshot?.phase === 'sudden-death' && !expectedResolution.suddenDeath) {
        await expect(
          page.getByRole('heading', { name: `Cliffhanger · Round ${nextSnapshot.round}` }),
        ).toBeVisible();
        ...
        expect(
          await cliffhanger.evaluate((record) => {
            ...
            return {
              ...
              insideSpeech:
                box.left >= speech.left &&
                box.top >= speech.top &&
                box.right <= speech.right &&
                box.bottom <= speech.bottom,
    ...
    await expect(page.locator('grand-transition-resolution-results')).toHaveCount(0);
  }
  ...
  for (const [playerId, result] of Object.entries(terminalResolution.players)) {
    await expect(
      page.locator(`[data-round-player="${playerId}"] .reaction-damage-total`),
    ).toContainText(new RegExp(`Final damage\\s*${String(result.outgoingDamage)}`, 'u'));
  }
  ...
  await expect(page.locator('grand-transition-resolution-results')).toHaveCount(0);
  await expect(page.getByRole('button', { name: /rematch|statistics/iu })).toHaveCount(0);
  ...
  const recordedPhraseEvidence = await (async () => {
    const [raw] = await storedHistory(page);
    const entry = raw
      ? (JSON.parse(raw) as {
          matchLog?: {
            sentences?: Array<{
    ...
    return {
      ...
      phrase: sentences.flatMap((candidate) => candidate.phrases)[0]?.text ?? '',
    };
  })();
  expect(recordedPhraseEvidence.sentence).not.toBe('');
  expect(recordedPhraseEvidence.phrase).not.toBe('');
  await expect(page.getByText(recordedPhraseEvidence.sentence, { exact: true })).toBeVisible();
...
async function executeDraftAction(page: Page, action: MatchBrowserAction): Promise<void> {
  ...
  switch (command.type) {
    case 'select-phrase': {
      ...
      await page
        .locator(`[data-card-source="${card.source}"][data-card-id="${card.cardId}"]`)
        .click();
      return;
    }
