  ...
  test(`${scene} fills ultrawide viewports without distorting scene layers`, async ({
    ...
    for (const viewport of [
      ...
      const geometry = await page.evaluate(async () => {
        ...
        const focalRectangles = JSON.parse(
          images[0]!.closest('picture')!.getAttribute('data-scene-focal-rectangles')!,
        ) as {
        ...
        return {
          ...
          covers:
            back.left <= 0.5 &&
            back.top <= 0.5 &&
            back.right >= innerWidth - 0.5 &&
            back.bottom >= innerHeight - 0.5,
          aligned: images.every((image) => {
            const box = image.getBoundingClientRect();
            return (
              Math.abs(box.x - back.x) < 0.5 &&
              Math.abs(box.y - back.y) < 0.5 &&
              Math.abs(box.width - back.width) < 0.5 &&
              Math.abs(box.height - back.height) < 0.5
            );
          }),
          undistorted: images.every((image) => {
            const box = image.getBoundingClientRect();
            return (
              Math.abs(box.width / box.height - image.naturalWidth / image.naturalHeight) < 0.001
            );
          }),
          noScroll:
