...
async function prepare(page: Page, lethal: boolean): Promise<number> {
  ...
  const draftingBubbleWidth = await page
...
for (const size of gateViewports([
  ...
  test(`recitation, inline scoring, impact, and automatic next round at ${size.width}x${size.height}`, async ({
    ...
    const draftingBubbleWidth = await prepare(page, false);
    ...
    const redName = await page.locator('grand-transition-match').evaluate(
      (element) =>
        (
    ...
    await expect
      .poll(() =>
        blue.evaluate(
          (element) => (element as unknown as { cue: { stateId: string } }).cue.stateId,
        ),
      )
      .toBe('delivery');
    ...
    expect(await impact.evaluate((element) => getComputedStyle(element).animationName)).toBe(
      'none',
    );
    ...
    await expect(redMeter).toHaveAttribute(
      'value',
      String(state.players['player-one']!.prideAfter),
    );
...
test('cliffhanger restoration stays inside the protected speech footprint at all supported viewports', async ({
  page,
}, info) => {
  await useFixedBrowserMatchSeed(page, 20260823);
  await page.goto('/grand-transition/');
    ...
    const geometry = await page.locator('.cliffhanger-strike').evaluate((record) => {
      ...
      const moderator = focal.moderatorFace
        ? {
            ...
            right:
              backgroundBox.left +
              backgroundBox.width * (focal.moderatorFace.x + focal.moderatorFace.width),
            top: backgroundBox.top + backgroundBox.height * focal.moderatorFace.y,
            bottom:
              backgroundBox.top +
              backgroundBox.height * (focal.moderatorFace.y + focal.moderatorFace.height),
          }
        : null;
      const overlaps =
        moderator &&
        box.left < moderator.right &&
        box.right > moderator.left &&
        box.top < moderator.bottom &&
        box.bottom > moderator.top;
      return {
...
test('terminal narration finishes for both characters before Victory can cover the arena', async ({
  page,
}) => {
...
test('long inline scores keep the latest line, bonuses, and total visible beside the speaker', async ({
  page,
}, info) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  ...
  await page.locator('grand-transition-match').evaluate(async (element) => {
    ...
    match.presentation = {
      ...
      components: Array.from({ length: 10 }, (_, index) => ({
        ...
        phraseText: `Your public statement number ${index + 1} failed the audit.`,
        base: 5,
        amount: 20,
        restrictionFactor: 1,
        weaknessFactor: 2,
  ...
  for (const size of [
    ...
    const geometry = await page.locator('.delivery-receipt').evaluate((record) => {
      ...
      const box = record.getBoundingClientRect();
      return {
        latestVisible: latest.top >= area.top - 1 && latest.bottom <= area.bottom + 1,
    ...
    await page.screenshot({
      path: info.outputPath(`speaker-inline-${size.width}x${size.height}.png`),
    });
  }
  ...
  await expect
    .poll(() =>
      page
        .locator('.delivery-components')
        .evaluate((list) => Math.abs(list.scrollHeight - list.clientHeight - list.scrollTop)),
    )
    .toBeLessThan(2);
});
