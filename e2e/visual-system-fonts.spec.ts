...
const speech =
  'A national-salvation committee repackages an infrastructure feasibility study, during the night, as thieves, before the microphones cool, and a county-council majority coordinates a public-procurement file, through another reform cycle, pending further consultation.';
...
test('Romanian display text and every diacritic use Poiret One glyphs', async ({ page }) => {
  ...
  for (const fonts of [titleFonts, nameFonts, rosterFonts]) {
    ...
    expect(
      fonts.every(({ isCustomFont, familyName }) => isCustomFont && familyName === 'Poiret One'),
...
for (const fallback of [false, true]) {
  test(`four font roles preserve complete text with ${fallback ? 'metric fallbacks' : 'local WOFF2 fonts'}`, async ({
    page,
    browser,
  }, testInfo) => {
    await page.setViewportSize({ width: 1024, height: 720 });
    ...
    await page.goto('');
    await expect(
      page.getByRole('heading', { name: 'Grand Transition', exact: true }),
    ).toBeVisible();
    await page.evaluate(() => document.fonts.ready);
    ...
    await page
      .locator(
        '.roster-choice[data-character-id="apartment-block-geopolitician"][data-skin-id="default"]',
      )
      .click();
    ...
    await page.locator('grand-transition-match').evaluate(
      async (element, fixtures) => {
        const match = element as HTMLElement & {
          snapshot: {
            revision: number;
            sentenceText: string;
            sharedCards: { text: string }[];
            privateCards: { text: string }[];
          };
    ...
    for (const [width, height] of [
      ...
      const facts = await page.evaluate(() => {
        const roles = [
          ['feature', '.match-player h2'],
          ['speech', '.sentence-preview'],
          ['interface', '.shared-board .card-phrase'],
          ['timer', '.timer-fact dd'],
          ['disabled', '.match-actions button:disabled .action-title'],
          ['diacritics', '.private-hand .card-phrase'],
        ];
        return {
          viewport: [innerWidth, innerHeight],
          fonts: [...document.fonts].map(({ family, weight, status }) => ({
            family,
            weight,
            status,
          })),
              ...
              fits:
                ...
                (role !== 'timer' ||
                  (textBox.top >= frameBox.top &&
                    ...
                    textBox.left >= frameBox.left &&
                    textBox.right <= frameBox.right)),
              inside:
      ...
      if (width === 1024 && height === 720)
        await page.screenshot({
          path: testInfo.outputPath(`fonts-${fallback ? 'fallback' : 'local'}-1024x720.png`),
        });
    }
    ...
    await testInfo.attach('font-comparison.json', {
