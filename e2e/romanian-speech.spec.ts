...
} from './helpers/romanian-speech.ts';
...
test('Romanian game speech uses the shipped Romanian voices and never leaves the device', async ({
  page,
}, info) => {
  ...
  await page.getByRole('button', { name: end, exact: true }).click();
  await expect(page.getByRole('heading', { name: /R(?:ound|unda) 2/u })).toBeVisible({
    timeout: 150_000,
  });
  const evidence = await page.evaluate(() => {
    ...
    return {
      ...
      pcm: (
        window as unknown as { romanianSpeechEvidence: Evidence }
      ).romanianSpeechEvidence.messages
        .filter(
          (
            message,
          ): message is {
            ...
            sampleRate: number;
            playbackRate: number;
            markers: { index: number; seconds: number }[];
  ...
  expect(
    evidence.speech.messages.filter((message) => (message as { type?: string }).type === 'error'),
  ).toEqual([]);
  expect(
    evidence.diagnostics.events.filter(
      (event) => event.type === 'error' || event.type === 'fallback',
    ),
  ).toEqual([]);
    ...
    expect(
      markers.every(
        ({ seconds }, index) =>
          ...
          seconds >= 0 &&
          seconds <= duration &&
          (index === 0 || seconds >= markers[index - 1]!.seconds),
      ),
    ).toBe(true);
  }
  for (const speakerId of ['player-one', 'player-two']) {
    ...
    const displayed = events
      .filter(({ type }) => type === 'presentation-segment')
      .map(({ segment }) => segment);
    expect(spoken).toEqual([0, 1, 2, 3, 4]);
    expect(displayed).toEqual(spoken);
    for (const [index, segment] of displayed.entries()) {
      const speechPosition = events.findIndex(
        (event) => event.type === 'segment' && event.segment === segment,
      );
      const displayPosition = events.findIndex(
        (event) => event.type === 'presentation-segment' && event.segment === segment,
      );
      expect(displayPosition, `${speakerId} score marker ${index}`).toBeGreaterThan(speechPosition);
    }
    expect(events.findIndex(({ type }) => type === 'presentation-total')).toBeGreaterThan(
      events.findLastIndex(({ type }) => type === 'presentation-segment'),
    );
  }
  ...
  const spoken = speech.neural
    .flatMap((call) => call.segments.join(' ').split(/\s+/u))
  ...
  await writeFile(
    info.outputPath('romanian-speech.json'),
    JSON.stringify({ evidence, requests: requests.length }, null, 2),
  );
});

test('a Romanian interface with an English game language keeps English speech', async ({
  page,
}) => {
  test.setTimeout(180_000);
  const errors: string[] = [];
  ...
  const evidence = await page.evaluate(
    () => (window as unknown as { romanianSpeechEvidence: Evidence }).romanianSpeechEvidence,
  );
...
async function playRound(
  ...
  evidence: Evidence;
  settings: { speechEnabled: boolean; gameLocale: string };
  errors: string[];
  diagnostics: import('../src/audio/speech-diagnostics.ts').SpeechDiagnosticsDocument;
}> {
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  ...
  return {
    evidence: await romanianSpeechEvidence(page),
    settings: await appSettings(page),
    errors,
    diagnostics,
  };
}

test('speech that is switched off never synthesizes Romanian or any platform voice', async ({
  page,
}) => {
  test.setTimeout(180_000);
  ...
  expect({ speechEnabled: settings.speechEnabled, gameLocale: settings.gameLocale }).toEqual({
    speechEnabled: false,
    gameLocale: 'ro-RO',
  });
...
test('corrupt Romanian weights keep the delivery silent without breaking the match', async ({
  page,
}) => {
  test.setTimeout(180_000);
  ...
  await page.route('**/tts/ro/**/model.onnx', (route) =>
    route.fulfill({
      status: 200,
  ...
  expect(
    diagnostics.events.some(({ type, reason }) => type === 'error' && reason === 'inference'),
  ).toBe(true);
...
test('a missing Romanian package keeps the delivery silent without breaking the match', async ({
  page,
}) => {
  test.setTimeout(180_000);
  await configure(page, { interfaceLocale: 'en', gameLocale: 'ro-RO' });
  ...
  expect(
    diagnostics.events.some(({ type, reason }) => type === 'error' && reason === 'worker'),
  ).toBe(true);
  expect(diagnostics.events.some(({ type }) => type === 'fallback')).toBe(false);
  expect(evidence.native).toEqual([]);
  expect(errors).toEqual([]);
...
test('Pause suspends Romanian playback and Resume completes the public delivery', async ({
  page,
}) => {
  test.setTimeout(180_000);
  await configure(page, { interfaceLocale: 'en', gameLocale: 'ro-RO' });
  ...
  await page.getByRole('button', { name: interfaceText.en.end, exact: true }).click();
  await expect
    .poll(async () => (await romanianSpeechEvidence(page)).neural.length, { timeout: 90_000 })
    .toBeGreaterThan(0);
...
test('Romanian inference timeout ends the public delivery without another voice', async ({
  page,
}) => {
  test.setTimeout(180_000);
  await configure(page, { interfaceLocale: 'en', gameLocale: 'ro-RO' });
  ...
  await page.evaluate(() => {
    ...
    nativePrototype.postMessage = function (
      message: unknown,
      options?: Transferable[] | StructuredSerializeOptions,
    ) {
    ...
    .poll(() =>
      page.evaluate(() =>
        (
          ...
          .snapshot()
          .events.some(({ type, reason }) => type === 'timeout' && reason === 'inference'),
      ),
    )
    .toBe(true);
...
test('visibility and menu navigation stop Romanian playback within 100 ms', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    return {
      sourceCount,
      stoppedAfterMs: stops.length ? stops.at(-1)! - began : null,
      remainingSources: app.speech.romanian.sources.size,
      matchEnded: app.matchState === null,
    };
  });
...
test('a long public Romanian sentence with all diacritics yields complete PCM and markers', async ({
  page,
}) => {
  test.setTimeout(180_000);
  await configure(page, { interfaceLocale: 'en', gameLocale: 'ro-RO' });
  ...
  const accepted = await page.evaluate((parts) => {
    const app = document.querySelector('grand-transition-app') as unknown as {
      speech: {
        speak(request: import('../src/audio/speech-port.ts').SpeechRequest): { accepted: boolean };
      };
    };
    const record = { markers: [] as number[], ended: false, error: false };
    Object.assign(window, { romanianLongSpeech: record });
    return app.speech.speak({
      text: parts.join(''),
      segments: parts,
      chunkStarts: [0],
      language: 'ro-RO',
      voiceUri: 'piper:ro_RO-mihai-medium',
      volume: 1,
  ...
  await expect
    ...
    .toMatchObject({ ended: true, error: false });
  const result = await page.evaluate(() => {
    const record = (window as unknown as { romanianLongSpeech: { markers: number[] } })
      .romanianLongSpeech;
    const messages = (window as unknown as { romanianSpeechEvidence: Evidence })
      .romanianSpeechEvidence.messages;
    ...
    return {
      delivered: record.markers,
      chunks: speech.length,
      ...
      native: (window as unknown as { romanianSpeechEvidence: Evidence }).romanianSpeechEvidence
        .native,
    };
  });
