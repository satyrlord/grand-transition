...
Replay versions 1 through 4 use normalized JSON with these fields in order:
...
breakdowns, public rule events, and winner. Version 2, version 3, and version 4
...
sentence record remain valid. Version 3 and version 4 logs require the complete
...
New replays and match logs use version 4. Decoding and replaying versions 1
through 3 selects each version's original scoring balance so retained commands
reproduce their original result.
...
retain the version 2 scoring result after version 4 becomes current. Encoding
preserves the supplied supported version and never relabels older commands as
version 4. Focused tests also preserve modifier-bearing version 3 scores.
...
- **AC-014-01:** Encoding, decoding, and re-encoding a replay produces identical
  ...
  scoring fixtures replay with their original resolutions after version 4 becomes
  current.
