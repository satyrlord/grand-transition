...
A missing human assignment defaults to George. A missing robot
...
outside David, Mark, and Zira.

Each assigned skin ID must exist in the character
asset catalog.
...
three-tense families.

Every scene-restricted card belongs to one scene only.
...
common board without a repeated phrase identifier.

Scene conjunctions are
...
text and number-form key exists.

Schemas reject HTML, script URLs, inline
...
- Neutral phrases use neutral identifiers and explicit empty `tags` arrays.
  ...
  in every tense. Family references keep their authored family weakness
...
- A phrase from real speech records its source in the private research folder
  and repeats the real wording. This accuracy permits faithful translation. An invented phrase needs no source record. Every common
  ...
  source URL, quoted wording, language, context, and affected card ID.
  ...
  nouns, verbs, conjunctions, and the continuation obey the general invented-or-real rule.
...
- Milestone 027 permits real slogans, real political speech, and documented
  memes. A direct real phrase keeps its real wording and meaning. Quote-inspired adaptations keep the source meaning without
...
cards. It contains 300 general nouns, 150 general verbs, 99 general
predicates, and 50 general modifiers. It also contains five general
conjunctions, 50 general endings, and one universal continuation. The verb pool has 50 past-tense, 50 present-tense,
...
the private research folder.

Each scene has exactly 34 scene-restricted cards
...
one tense fails humor review, the complete family changes together.
...
English context. Each player-visible phrase text and agreement form contains no more than 11
whitespace-delimited words. It also meets its role limit: conjunction 6,
continuation 1, verb 10, modifier 9, noun 10, predicate 10, and ending 11. Comeback lines stay at 16 words or fewer. The manual card parser
...
ceiling rather than inside it. The longest shipped comeback has 14 words. Its Romanian adaptation has 13
words. The limit leaves a small margin. Changes to either language are not necessary.

The limits use the measured corpus of the source party game that the
interface imitates. Its connectors have one word. Objects and predicates have
a median of three to four words. Finishers have about six words. The longest
comeback has eleven words.

In that style, a card can exceed the measured range when extra words supply
the comic image or punchline. Humor has priority over brevity. A later
translation may need more words than the English original. The tighter editorial bands stay in the private
...
`legacy` weakness tag. Keep this exact approved wording. Do not claim that
...
keys. It also derives the English message table. It derives no weakness locale key. Milestone 029 owns displayed weakness
labels as interface copy. Scoring and stored state keep the stable tag
...
system and pass them to the same pure parser. Only one new character JSON file and its approved
`src/assets/characters/<character-id>.png` portrait are necessary to add a
character. No TypeScript import, registry, locale, setup, or renderer edit is
necessary. Content, localization, asset,
...
- **AC-005-07:** The common corpus contains every required phrase role. It
  ...
  always `[...]`. It remains neutral. Quote provenance is not necessary.
...
slogan](https://www.rri.ro/en/news-and-current-affairs/the-week-in-review/29-january-4-february-2017-id124467.html)
  ...
  another language can reproduce it faithfully. It keeps its stable identifier
...
- **AC-005-09:** A synthetic character supplied as one correctly named JSON
  ...
  selects the character in setup, and does checks of its loaded portrait in a match. It then
  removes both files, makes a second clean build, and shows that the setup,
  match, and production output contain no temporary character.

## Objective verifier

`tests/unit/content-schemas.test.ts` does checks of AC-005-01 through AC-005-09 for
pure authoring and browser discovery. `tests/unit/node-content-discovery.test.ts`
does checks of Node discovery parity. `e2e/content-lifecycle.spec.ts` does checks of the
