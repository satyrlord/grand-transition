import type { MatchResolutionPlayer } from '../engine/match-lifecycle.ts';
import type { SettingsDocument } from '../persistence/codecs/settings-codec.ts';
import type { SpeechPort, SpeechRequest } from './speech-port.ts';
import { mixerGains } from './audio-port.ts';
import type { SkinSpeechProfile } from './skin-speech-profile.ts';
import {
  reportSpeech,
  type PublicSpeechEvent,
  type SpeechCancellationReason,
  type SpeechDiagnostic,
} from './speech-diagnostics.ts';

export type SpeechDeliveryEvents = Pick<
  SpeechRequest,
  'onStart' | 'onSegment' | 'onEnd' | 'onError'
>;
...
export function publicNarrationSegments(player: MatchResolutionPlayer): readonly string[] {
  ...
  for (const phrase of player.constructionPhrases) {
    ...
    if (position < 0)
      throw new Error('Final public phrase positions do not match the completed insult.');
    positions.push(position);
    cursor = position + phrase.text.trim().length;
  }
  const segments = positions.length
    ? positions.map((position, index) =>
        text.slice(index === 0 ? 0 : position, positions[index + 1] ?? text.length),
      )
    : [text];
...
export function publicNarrationChunkStarts(player: MatchResolutionPlayer): readonly number[] {
  ...
  for (const item of player.score?.breakdown ?? []) {
    ...
    for (const phraseId of item.phraseIds) {
      const index = player.constructionPhrases.findIndex(
        (phrase, index) => index > end && phrase.phraseId === phraseId,
      );
      if (index < 0)
        return Object.freeze(
          player.comebackClosingLine ? [...starts, player.constructionPhrases.length] : starts,
        );
      end = index;
    }
    if (previousEnd >= 0 && previousEnd + 1 < player.constructionPhrases.length)
      starts.push(previousEnd + 1);
    previousEnd = end;
    cursor = end + 1;
  }
  if (player.comebackClosingLine && player.constructionPhrases.length > 0)
    starts.push(player.constructionPhrases.length);
...
export class GameSpeech {
  ...
  private readonly diagnostic: (event: PublicSpeechEvent) => void;

  constructor(speech: SpeechPort, diagnostic: (event: PublicSpeechEvent) => void = () => {}) {
    this.speech = speech;
  ...
  presentation(
    ...
    profile: SkinSpeechProfile,
    event: SpeechDiagnostic,
  ): void {
    reportSpeech(
      {
        onDiagnostic: (detail) =>
          this.diagnostic({
            round,
            speakerId,
            voice: profile.microsoftVoice ?? profile.voiceUri,
            rate: settings.speechRate,
            pitch: profile.pitch,
            provider: profile.provider,
            ...(round === this.diagnosticRound ? this.actualVoices.get(speakerId) : undefined),
            ...detail,
          }),
      },
      event,
  ...
  private request(
    ...
    const onDiagnostic: NonNullable<SpeechRequest['onDiagnostic']> = (event) => {
      ...
      if (round !== this.diagnosticRound) {
        this.actualVoices.clear();
        this.diagnosticRound = round;
      }
      if (event.voice && event.provider)
        this.actualVoices.set(player.playerId, { voice: event.voice, provider: event.provider });
      this.diagnostic({
        round,
        speakerId: player.playerId,
        voice:
          event.provider === 'neural'
            ? profile.voiceUri
            : (profile.microsoftVoice ?? profile.voiceUri),
        rate: settings.speechRate,
        pitch: profile.pitch,
        provider: profile.provider,
        ...event,
    ...
    if (
      ...
      !this.speech.available ||
      mixerGains(settings).speech === 0 ||
      !player.completeValidInsult ||
      player.constructionStatus !== 'valid' ||
      !player.insultText?.trim()
    ) {
      reportSpeech(
        { onDiagnostic },
        {
          type: 'skipped',
          reason:
            !player.completeValidInsult || player.constructionStatus !== 'valid'
              ? 'incomplete'
              : !settings.speechEnabled
                ? 'disabled'
                : mixerGains(settings).speech === 0
    ...
    try {
      segments = publicNarrationSegments(player);
    } catch (error) {
      reportSpeech({ onDiagnostic }, { type: 'error', reason: 'segmentation' });
      throw error;
    }
    return {
      text,
      segments,
      chunkStarts: publicNarrationChunkStarts(player),
      ...profile,
      rate: settings.speechRate,
      volume: mixerGains(settings).speech,
      onDiagnostic,
    };
  }
