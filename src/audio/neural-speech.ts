...
} from './speech-diagnostics.ts';
import { createPiperClarity } from './piper-clarity.ts';

export type NeuralSpeechStatus =
  'idle' | 'loading' | 'ready' | 'generating' | 'speaking' | 'unavailable';
type Dependencies = {
  supported: () => boolean;
...
type Pending = {
  ...
  playback: boolean;
  key: string;
  parts: { segments: readonly string[]; offset: number }[];
  prepared: Prepared[];
  generated: number;
  scheduled: number;
  completed: number;
...
function preparationKey(request: SpeechRequest): string {
  return JSON.stringify([
    request.text,
    request.segments ?? [request.text],
    request.language,
    request.voiceUri ?? null,
    request.rate ?? 1,
    request.pitch ?? 1,
    request.chunkStarts ?? [0],
  ]);
}
...
export class LocalNeuralSpeech implements SpeechPort {
  ...
  constructor(
    ...
    dependencies: Dependencies = {
      ...
      createWorker: () =>
        new Worker(new URL('./neural-speech-worker.ts', import.meta.url), { type: 'module' }),
  ...
  get available(): boolean {
    return !this.disposed && this.status !== 'unavailable' && this.dependencies.supported();
  }

  initialize(activate = true): Promise<boolean> {
    if (this.disposed || !this.dependencies.supported()) {
  ...
  private enqueue(request: SpeechRequest, playback: boolean): SpeechResult {
    ...
    if (
      !(
        this.dependencies.acceptsLanguage ??
    ...
    const existing = [...this.pending.values()].find(
      (pending) => !pending.playback && pending.key === key,
    );
    if (existing) {
      if (playback) {
        existing.request = request;
        existing.playback = true;
        this.pump();
      }
    ...
    const parts = starts.map((offset, index) => ({
      offset,
      segments: segments.slice(offset, starts[index + 1] ?? segments.length),
    }));
    // Keep at most one future delivery. It is local to this presentation, not a persistent cache.
    for (const [id, pending] of this.pending)
      if (!pending.playback) {
        reportSpeech(pending.request, {
          type: 'cancel',
          provider: 'neural',
          reason: 'replacement',
        });
  ...
  cancel(reason: SpeechCancellationReason = 'replacement'): void {
    ...
    for (const pending of this.pending.values())
      reportSpeech(pending.request, { type: 'cancel', provider: 'neural', reason });
    this.pending.clear();
    this.stopSource();
  ...
  pause(): void {
    if (!this.paused)
      for (const pending of this.pending.values())
        reportSpeech(pending.request, { type: 'pause', provider: 'neural' });
    this.paused = true;
  ...
  resume(): void {
    if (this.paused)
      for (const pending of this.pending.values())
        reportSpeech(pending.request, { type: 'resume', provider: 'neural' });
    this.paused = false;
    if (this.context)
      void this.context
        .resume()
  ...
  private send(message: NeuralSpeechCommand): void {
  ...
  private watchInitialization(): void {
    ...
    this.initializationWatchdog = setTimeout(
      () => this.fail('worker'),
      this.dependencies.initializationTimeoutMs,
    );
  }
  ...
  private dispatch(): void {
    ...
    const next =
      entries.find(([, pending]) => pending.playback && pending.generated < pending.parts.length) ??
    ...
    const voice =
      this.voices.find(({ voiceURI }) => voiceURI === request.voiceUri) ??
      this.voices.find(
        (candidate) => candidate.lang.toLowerCase() === language && candidate.default,
      ) ??
      this.voices.find((candidate) => candidate.lang.toLowerCase() === language) ??
      this.voices.find((candidate) => candidate.default) ??
      this.voices[0];
    ...
    this.send({
      ...
      voiceId:
        voice?.voiceURI.replace(this.dependencies.voicePrefix ?? 'piper:', '') ??
        this.dependencies.defaultVoiceId ??
        'vctk-p226',
      rate: request.rate ?? 1,
  ...
  private readonly receive = ({ data }: MessageEvent<NeuralSpeechMessage>): void => {
    ...
    if (data.type === 'booted') {
      ...
      this.watchInitialization();
      const progress =
        data.total > 0 ? Math.min(1, Math.floor((data.loaded / data.total) * 100) / 100) : null;
      ...
      for (const pending of this.pending.values())
        reportSpeech(pending.request, { type: 'model-ready', provider: 'neural' });
      ...
      if (data.type === 'error') {
        if (data.id === null || (current && this.pending.has(active!.requestId))) {
          this.fail(data.id === null ? 'worker' : 'inference');
          return;
        }
      } else if (current) {
        ...
        if (pending) {
          reportSpeech(pending.request, {
            type: 'pcm-ready',
            provider: 'neural',
            durationMs: (data.samples.length / data.sampleRate / data.playbackRate) * 1000,
          });
          pending.generated++;
          pending.prepared.push({
            ...
            markers: data.markers.map((marker) => ({
              ...marker,
              index: marker.index + active!.offset,
            })),
          });
          this.pump();
        }
  ...
  private pump(): void {
    ...
    if (pending.completed === pending.parts.length) {
      ...
      this.playbackEnd = 0;
      this.setStatus(
        [...this.pending.values()].some((value) => value.playback) ? 'generating' : 'ready',
      );
    ...
    for (const data of pending.prepared.splice(0)) {
      if (
        data.sampleRate !== (this.dependencies.sampleRate ?? 22050) ||
        !data.samples.length ||
        !Number.isFinite(data.playbackRate) ||
        data.playbackRate < 0.5 ||
        data.playbackRate > 2
      ) {
        this.fail();
        return;
      ...
      try {
        ...
        source.connect(gain);
        gain.connect(this.clarity?.input ?? context.destination);
        ...
        const began = Math.max(
          context.currentTime + (needsBuffer ? (this.dependencies.startupBufferSeconds ?? 0) : 0),
          this.playbackEnd,
        );
        ...
        const notifyAt = (seconds: number, callback: () => void) => {
          ...
          const check = () => {
            ...
            if (remaining > 0.002) {
              schedule(check, Math.max(4, remaining * 1000));
              return;
            }
            callback();
          };
        ...
        if (firstChunk)
          notifyAt(0, () => reportSpeech(request, { type: 'playback-start', provider: 'neural' }));
        if (firstChunk)
          notifyAt(data.markers[0]?.seconds ?? 0, () => {
            this.setStatus('speaking');
            request.onStart?.();
          });
        for (const marker of data.markers)
          notifyAt(marker.seconds, () => {
  ...
  private stopSource(): void {
    ...
    for (const [source, gain] of this.sources) {
      ...
      try {
        ...
        /* The completed source is already silent. */
      }
      source.disconnect();
      gain.disconnect();
    }
  ...
  private fail(reason: SpeechDiagnostic['reason'] = 'audio'): void {
    ...
    const requests = [...this.pending.values()]
      .filter((pending) => pending.playback)
      .map(({ request }) => request);
    this.cancel('failure');
    clearTimeout(this.watchdog);
    this.synthesizing = null;
