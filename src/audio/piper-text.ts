...
export function piperControls(
  rate = 1,
  pitch = 1,
): { playbackRate: number; durationScale: number } {
  if (
...
export function piperMarkers(
  ...
  if (offsets.at(-1) !== samples || starts.some((start) => start >= durations.length))
    throw new Error('Speech timing does not match the generated audio.');
  return starts.map((start, index) => ({
    index,
    seconds: offsets[start]! / sampleRate / playbackRate,
  }));
}
