...
export const speechDiagnosticsSchema = z
  .object({
    schemaVersion: z.literal(1),
    status: z.enum(['recording', 'finished', 'interrupted']),
    droppedEvents: z.number().int().nonnegative(),
    events: z
      .array(publicSpeechEventSchema.extend({ elapsedMs: z.number().int().nonnegative() }))
      .max(1000),
  })
  .strict();
...
export function reportSpeech(
  request: { onDiagnostic?: (event: SpeechDiagnostic) => void },
  event: SpeechDiagnostic,
): void {
  try {
    request.onDiagnostic?.(event);
  } catch {
    /* Observation cannot interrupt playback. */
  }
}

export class SpeechDiagnostics {
  private began = 0;
  private document: SpeechDiagnosticsDocument = {
    schemaVersion: 1,
    status: 'recording',
    droppedEvents: 0,
    events: [],
  ...
  capture(event: PublicSpeechEvent): void {
    ...
    if (this.document.events.length === 1000) {
      this.document.events.shift();
      this.document.droppedEvents++;
    }
    this.document.events.push({
      ...parsed.data,
      elapsedMs: Math.max(0, Math.round(this.now() - this.began)),
    });
  }
  finish(interrupted = false): void {
    if (this.document.status === 'recording')
      this.document.status = interrupted ? 'interrupted' : 'finished';
  }
  snapshot(): SpeechDiagnosticsDocument {
