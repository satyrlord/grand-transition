...
async function load(baseUrl: string): Promise<void> {
  ...
  if (
    ...
    !base.pathname.endsWith('/tts/piper/') ||
    base.search ||
    base.hash
  ) {
  ...
  if (
    config.audio.sample_rate !== manifest.sampleRate ||
    Object.values(config.phoneme_id_map).some((ids) => ids.length !== 1)
  )
    throw new Error('Unsupported phoneme map.');
  ...
  session = await InferenceSession.create(await readAsset('model.onnx', true), {
...
async function synthesize(request: Extract<NeuralSpeechCommand, { type: 'synthesize' }>) {
  ...
  while (cursor < phones.length) {
    ...
    const feeds = {
      ...
      scales: new Tensor(
        ...
        new Float32Array([
          ...
          config.inference.length_scale * durationScale,
    ...
    const timing = piperMarkers(
      durations,
      inside.map((start) => 2 + (start.phone - cursor) * 2),
      waveform.length,
      manifest.sampleRate,
      playbackRate,
    );
    for (const [index, marker] of timing.entries())
      markers.push({
        index: inside[index]!.index,
