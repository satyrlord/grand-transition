...
export function selectMicrosoftRobotVoice(
  voices: readonly SpeechSynthesisVoice[],
  name: 'David' | 'Mark' | 'Zira',
): SpeechSynthesisVoice | undefined {
  return voices
    .filter(
      (voice) =>
        voice.localService &&
        /^en(?:-|$)/iu.test(voice.lang) &&
        new RegExp(`^Microsoft\\s+${name}\\b`, 'iu').test(voice.name) &&
        !/online|natural|neural/iu.test(voice.name),
    )
...
export class MicrosoftRobotSpeech implements SpeechPort {
  ...
  private voice(name: 'David' | 'Mark' | 'Zira' = 'David'): SpeechSynthesisVoice | undefined {
    try {
      return selectMicrosoftRobotVoice(this.dependencies.service()?.getVoices() ?? [], name);
    } catch {
      return undefined;
    }
  }
  ...
  get available(): boolean {
    return Boolean(this.voice('David') || this.voice('Mark') || this.voice('Zira'));
  }
      ...
      try {
        ...
        utterance.onboundary = (event) => {
          if (
            ...
            event.charIndex < 0 ||
            event.charIndex >= request.text.length
          )
            return;
        ...
        reportSpeech(request, {
          ...
          voice: request.microsoftVoice ?? 'David',
        });
  ...
  cancel(reason: SpeechCancellationReason = 'replacement'): void {
    ...
    if (this.request)
      reportSpeech(this.request, { type: 'cancel', provider: 'microsoft-local', reason });
    this.generation++;
    this.request = null;
    this.next = null;
    this.service = null;
    ...
    if (owned) {
      try {
        service?.cancel();
        if (this.paused) service?.resume();
      } catch {
        /* The failed service is already unavailable. */
      }
    }
  }
  ...
  private clearUtterance(): void {
    ...
    this.timeout = undefined;
    if (this.active) {
      this.active.onstart = null;
      this.active.onboundary = null;
