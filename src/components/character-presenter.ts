...
export class GrandTransitionCharacter extends LitElement {
  static properties = {
    ...
    cue: { attribute: false },
    restState: { attribute: false },
    paused: { type: Boolean },
    displayedState: { state: true },
    motionRevision: { state: true },
  };
  ...
  declare private displayedState: CharacterStateId;
  declare private motionRevision: number;
  ...
  protected override willUpdate(changed: PropertyValues<this>): void {
    ...
    if (cueKey !== this.cueKey) {
      ...
      this.stopTimer();
      this.requestedState =
        this.suspended || !this.cue || this.cue.stateId === 'idle'
          ? this.restState
          : this.cue.stateId;
      this.showDecodedRequest(true);
    } else if (
      changed.has('restState') &&
      (this.requestedState === 'idle' || this.requestedState === 'thinking')
    ) {
    ...
    return html`<div class="character-state-layer" aria-hidden="true"
      ...
      ${this.frames.map((frame) => {
        ...
        return guard(
          ...
          ${
            decodedUrl
              ? html`<div class="character-state-upper" data-character-part="upper"
            style=${styleMap({ backgroundImage: `url("${decodedUrl}")` })}></div>`
              : nothing
          }
          </div>
        </div>`,
