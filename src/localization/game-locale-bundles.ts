import { isGameLocale, type GameLocale } from './game-locale';
...
export type GameLocaleBundles = Readonly<
  Partial<Record<GameLocale, GameLocaleBundle>>
>;

export function indexGameLocaleBundles(
  ...
  for (const bundle of bundles) {
    if (!isGameLocale(bundle.locale)) {
      throw new Error(
        `Game-locale bundle "${bundle.locale}" is not a shipped game locale.`,
      );
    }
    if (indexed[bundle.locale]) {
      throw new Error(
        `Ship one game-locale bundle per locale; "${bundle.locale}" is duplicated.`,
      );
    }
...
export function shippedGameLocales(
  bundles: GameLocaleBundles,
): readonly GameLocale[] {
