...
const packages = createCharacterStatePackages(
  stateManifest,
  characterAssetManifest,
  stateVariantUrls,
);
const packagesBySkin = new Map(
  packages.map((entry) => [entry.ownerId + ':' + entry.skinId, entry.frames]),
);

export function resolveCharacterFrames(
  ownerId: string,
  skinId: string,
...
export function createCharacterStatePackages(
  ...
  return Object.freeze(
    value.packages.map((raw) => {
      if (!isRecord(raw) || !Array.isArray(raw.states))
        throw new Error('Character state package must contain states.');
      ...
      seen.add(key);
      if (raw.states.length !== contract.states.length)
        throw new Error(key + ': all nine states are required.');
      const selection = selections.find(
        (asset) => asset.ownerId === ownerId && asset.skinId === skinId,
      );
      ...
      const frames = contract.states.map(({ id }) => {
        ...
        if (
          ...
          mapping.loop !== characterMotion[stateId].loop
        )
          throw new Error(key + ': invalid state mapping for ' + stateId);
        if (stateId === 'selection') {
          if (mapping.assetId !== selection.id)
            throw new Error(key + ': incorrect selection mapping.');
          return Object.freeze({
            id: selection.id,
            stateId,
            url: selection.url,
        ...
        const expectedAssetId =
          reusedStateId === 'selection'
            ? selection.id
            : reusedStateId
              ? selection.id + '--' + reusedStateId
...
function readVariants(
  ...
  value: unknown,
  urls: Readonly<Record<string, string>>,
): readonly CharacterAssetVariant[] {
  if (!Array.isArray(value) || value.length !== 6)
    throw new Error(id + ': six state variants are required.');
  const seen = new Set<string>();
  return Object.freeze(
    value.map((raw) => {
      if (
        ...
        ![320, 640, 960].includes(raw.width) ||
        raw.height !== raw.width ||
        (raw.format !== 'avif' && raw.format !== 'webp')
      ) {
      ...
      seen.add(key);
      const variantPath =
        'states/variants/' + id + '-' + raw.width + 'x' + raw.width + '.' + raw.format;
